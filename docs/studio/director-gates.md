...
```
Spawn `ccgs-creative-director` via Codex subagent using gate **CD-PILLARS** from
...
Set once during `$ccgs-start`. Edit the file directly to change it at any time.
...
Examples:
```
$ccgs-brainstorm space horror           → uses global mode
$ccgs-brainstorm space horror --review full   → forces full mode this run
$ccgs-architecture-decision --review solo     → skips all gates this run
```
...
| `lean` | PHASE-GATEs only (`$ccgs-gate-check`) — per-skill gates skipped | **Default** — solo devs and small teams; directors review at milestones only |
...
Spawn `[agent-name]` via Codex subagent:
...
Spawn all [N] agents simultaneously via Codex subagent — issue all Codex subagent calls before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `Codex request_user_input` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
Agent: `ccgs-creative-director` | Model tier: Opus | Domain: Vision, pillars, player experience
...
**Trigger**: After the systems index is written by `$ccgs-map-systems` — validates the
...
documents are authored (team-narrative, design-system for story systems, ccgs-writer
deliverables)
...
**Trigger**: After playtest reports are generated (`$ccgs-playtest-report`), or after
...
**Trigger**: Always at `$ccgs-gate-check` — spawn in parallel with TD-PHASE-GATE and PR-PHASE-GATE
...
Agent: `ccgs-technical-director` | Model tier: Opus | Domain: Architecture, engine risk, performance

---

### TD-SYSTEM-BOUNDARY — System Boundary Architecture Review

**Trigger**: After `$ccgs-map-systems` Phase 3 dependency mapping is agreed but before
...
**Trigger**: After the master architecture document is drafted (`$ccgs-create-architecture`
...
**Trigger**: After an individual ADR is authored (`$ccgs-architecture-decision`), before
...
**Trigger**: Always at `$ccgs-gate-check` — spawn in parallel with CD-PHASE-GATE and PR-PHASE-GATE
...
Agent: `ccgs-producer` | Model tier: Opus | Domain: Scope, timeline, dependencies, production risk
...
**Trigger**: Before finalising a sprint plan (`$ccgs-sprint-plan`), and after any
...
**Trigger**: At milestone review (`$ccgs-milestone-review`), at mid-sprint retrospectives,
...
**Trigger**: After epics are defined by `$ccgs-create-epics`, before stories are
broken out — validates the epic structure is producible before `$ccgs-create-stories`
is invoked
...
**Trigger**: Always at `$ccgs-gate-check` — spawn in parallel with CD-PHASE-GATE and TD-PHASE-GATE
...
Agent: `ccgs-art-director` | Model tier: Sonnet | Domain: Visual identity, art bible, visual production readiness
...
**Trigger**: After the art bible is drafted (`$ccgs-art-bible`), before asset production begins
...
**Trigger**: Always at `$ccgs-gate-check` — spawn in parallel with CD-PHASE-GATE, TD-PHASE-GATE, and PR-PHASE-GATE
...
**Trigger**: After the master architecture document is written (`$ccgs-create-architecture`
...
**Trigger**: After a dev story is implemented (`$ccgs-dev-story`, `$ccgs-story-done`), or
as part of `$ccgs-code-review`
...
**Trigger**: Before a story is accepted into a sprint — invoked by `$ccgs-create-stories`,
`$ccgs-story-readiness`, and `$ccgs-sprint-plan` during story selection
...
done, or at `$ccgs-gate-check` Production → Polish
...
**Trigger**: After ccgs-writer deliverables (dialogue, lore, item descriptions) are
...
at `$ccgs-gate-check`), spawn all agents simultaneously:

```
Spawn in parallel (issue all Codex subagent calls before waiting for any result):
1. ccgs-creative-director  → gate CD-PHASE-GATE
2. ccgs-technical-director → gate TD-PHASE-GATE
3. ccgs-producer           → gate PR-PHASE-GATE
4. ccgs-art-director       → gate AD-PHASE-GATE
...
1. Assign a gate ID: `[DIRECTOR-PREFIX]-[DESCRIPTIVE-SLUG]`
   ...
   - Add new prefixes for new agents: `ccgs-audio-director` → `AU-`, `ccgs-ux-designer` → `UX-`
