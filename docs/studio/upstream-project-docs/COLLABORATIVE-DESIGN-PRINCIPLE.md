...
11. AGENT WRITES FILE
    Agent: [Uses Write tool]
           "Created design/gdd/crafting-system.md. Would you like me to run
            $ccgs-design-review to validate it against the standard?"
```

---

## 📋 How This Applies to Different Codex subagents

### 🎨 Design Codex subagents
...
### 🎯 Brainstorming Codex subagents

**Example:** `$ccgs-brainstorm roguelike`
...
## 🎛️ Structured Decision UI (Codex request_user_input)

Use the `Codex request_user_input` tool to present decisions as a **selectable UI** instead
...
2. **Capture the decision** — Call `Codex request_user_input` with concise option labels
   and short descriptions. The user picks from the UI or types a custom answer.

### When to Use Codex request_user_input
...
- When running as a Codex subagent subagent (tool may not be available)
...
(main session) calls `Codex request_user_input` at each decision point between phases:

```
[ccgs-game-designer returns 3 combat approaches with analysis]

Orchestrator uses Codex request_user_input:
...
3. IF User says "Yes":
   Agent: [Uses apply_patch/edit tool]
...
```
User: "$ccgs-team-combat 'grappling hook ability'"
 ...
 **Phase 1: Design (ccgs-game-designer)**
 Starting design phase...
 [ccgs-game-designer asks questions, presents options]
 [User makes decisions]
 ccgs-game-designer: 'Design complete. Proceeding to architecture phase.'

 **Phase 2: Architecture (ccgs-gameplay-programmer)**
 [ccgs-gameplay-programmer proposes code structure]
 [User approves or requests changes]

 **Phase 3: Parallel Implementation**
 I'll now coordinate 4 agents to implement in parallel:
 - ccgs-gameplay-programmer: Core grapple physics
 - ccgs-technical-artist: Cable VFX
 - ccgs-sound-designer: Whoosh + impact SFX
 - ccgs-ai-programmer: Enemy reactions to being grappled
...
"All 4 subsystems implemented. Would you like me to:
 A) Have ccgs-gameplay-programmer integrate them now
 B) Let you test each independently first
 C) Run $ccgs-code-review before integration?"
```
...
- **Codex request_user_input tool** — Integrated into 16 skills for structured option UI
