...
async void LoadScene() {
    ...
    // Unload scene
    await Addressables.UnloadSceneAsync(handle).Codex subagent;
}
```
...
async Codex subagent<GameObject> GetAsset(string key) {
...
async void CheckForUpdates() {
    ...
    if (handle.Result.Count > 0) {
        ...
        await Addressables.UpdateCatalogs(handle.Result).Codex subagent;
    }
...
var handle = await Addressables.LoadAssetAsync<GameObject>("Enemies/Goblin").Codex subagent;
GameObject prefab = handle.Result;
```
