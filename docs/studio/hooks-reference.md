...
Hooks are configured in `.codex/hooks.json and .codex/rules` and fire automatically:
...
| `validate-assets.sh` | PostToolUse (apply_patch/edit) | Asset file changes | Checks naming conventions and JSON validity for files in `assets/` |
...
| `detect-gaps.sh` | SessionStart | Session begins | Detects fresh projects (suggests $ccgs-start) and missing documentation when code/prototypes exist, suggests $ccgs-reverse-document or $ccgs-project-stage-detect |
...
| `validate-skill-change.sh` | PostToolUse (apply_patch/edit) | Skill file changes | Advises running `$ccgs-skill-test` after any `.agents/skills/` file is written or edited |
