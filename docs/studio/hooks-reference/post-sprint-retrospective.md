...
Manual trigger at the end of each sprint (typically invoked by the ccgs-producer
...
```
@ccgs-producer Generate sprint retrospective for Sprint [N]
```

The ccgs-producer agent should:

1. **Read the sprint plan** from `production/sprints/sprint-[N].md`
2. **Calculate metrics**:
   - Codex subagents planned vs completed
...
| Metric | Value |
|--------|-------|
| Codex subagents Planned | [N] |
| Codex subagents Completed | [N] |
| Completion Rate | [X%] |
| Carryover from Previous | [N] |
| New Codex subagents Added | [N] |
| Bugs Found | [N] |
| Bugs Fixed | [N] |
