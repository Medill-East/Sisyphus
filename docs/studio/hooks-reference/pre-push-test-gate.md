...
1. Build failure: invoke `ccgs-lead-programmer` to diagnose
2. Unit test failure: invoke `ccgs-qa-tester` to identify the failing test and
   `ccgs-gameplay-programmer` or relevant programmer to fix
3. Performance regression: invoke `ccgs-performance-analyst` to analyze
