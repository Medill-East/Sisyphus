...
1. For style violations: auto-fix with your formatter or invoke `ccgs-lead-programmer`
2. For hardcoded values: invoke `ccgs-gameplay-programmer` to externalize the values
3. For test failures: invoke `ccgs-qa-tester` to diagnose and `ccgs-gameplay-programmer` to fix
