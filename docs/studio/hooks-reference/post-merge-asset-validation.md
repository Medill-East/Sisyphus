...
if [ -n "$WARNINGS" ]; then
    ...
    echo -e "$WARNINGS"
    echo "================================"
    echo "Run $ccgs-asset-audit for a full report."
fi
...
1. For naming violations: fix manually or invoke `ccgs-art-director` for guidance
2. For size violations: invoke `ccgs-technical-artist` for optimization advice
3. For a full audit: run `$ccgs-asset-audit` skill
