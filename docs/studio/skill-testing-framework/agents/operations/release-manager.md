# Agent Test Spec: ccgs-release-manager
...
- **Does NOT own**: Game design decisions, QA test strategy or test case design (ccgs-qa-lead), QA test execution (ccgs-qa-tester), build infrastructure (ccgs-devops-engineer)
- **Model tier**: Sonnet
- **Gate IDs**: May be invoked by `$ccgs-gate-check` during Release phase; LAUNCH BLOCKED verdict is ccgs-release-manager's primary escalation output
...
- States clearly: "Test case design is owned by ccgs-qa-lead (strategy) and ccgs-qa-tester (execution); I can provide the certification requirements that the save system must meet, which ccgs-qa-lead can then use to design tests"
...
- Escalates to ccgs-producer: documents the delay impact on release timeline
...
- Applies semantic versioning resolution: one must be re-tagged — the release branch changes should become v1.2.2 if v1.2.1 is already published; if v1.2.1 is not yet published, coordinate with ccgs-devops-engineer to merge or re-tag
...
- [ ] Redirects test case design requests to ccgs-qa-lead/ccgs-qa-tester without producing test specs
