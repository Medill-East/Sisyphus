# Agent Test Spec: ccgs-network-programmer

## Agent Summary
Domain: Multiplayer networking, state replication, lag compensation, matchmaking protocol design, and network message schemas.
Does NOT own: gameplay logic (only the networking of it), server infrastructure and deployment (ccgs-devops-engineer).
...
- Does NOT implement the player movement logic itself (defers to ccgs-gameplay-programmer)
...
- Explicitly states that server infrastructure belongs to `ccgs-devops-engineer`
- Redirects the request to `ccgs-devops-engineer`
...
- Does NOT change the physics simulation itself — documents the interface contract for ccgs-engine-programmer
...
- Coordinates with `ccgs-security-engineer` to agree on the validation boundary
- Proposes a compromise (server validates position within a tolerance band, flags outliers) rather than unilaterally deciding
- Documents the trade-off and escalates the final decision to `ccgs-technical-director` if ccgs-security-engineer and ccgs-network-programmer cannot agree
...
- Flags that rollback netcode requires ccgs-gameplay-programmer to implement deterministic simulation
...
- [ ] Redirects server deployment to ccgs-devops-engineer
...
- [ ] Coordinates with ccgs-security-engineer on anti-cheat boundaries
- [ ] Designs to explicit latency targets from provided context

---

## Coverage Notes
- Replication strategy (Case 1) should include a bandwidth calculation reviewable by ccgs-technical-director
- Rollback/reconciliation (Case 3) must document the ccgs-engine-programmer interface contract clearly
