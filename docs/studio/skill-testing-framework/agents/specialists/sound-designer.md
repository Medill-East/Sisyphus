# Agent Test Spec: ccgs-sound-designer

## Agent Summary
Domain: SFX specs, audio events, mixing parameters, and sound category definitions.
Does NOT own: music composition direction (ccgs-audio-director), code implementation of audio systems.
...
- Explicitly states that music direction belongs to `ccgs-audio-director`
- Redirects the request to `ccgs-audio-director`
...
- Flags the conflict to `ccgs-audio-director` with the proposed compliant alternative
- Will proceed with the corrected name once confirmed by ccgs-audio-director
...
- [ ] Redirects music direction requests to ccgs-audio-director
