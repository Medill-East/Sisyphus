# Agent Test Spec: ccgs-tools-programmer

## Agent Summary
Domain: Editor extensions, content authoring tools, debug utilities, and pipeline automation scripts.
Does NOT own: game code (ccgs-gameplay-programmer, ccgs-ui-programmer, etc.), engine core systems (ccgs-engine-programmer).
...
- Waypoints are serialized as engine-native resource (not hardcoded) so ccgs-level-designer can edit without code
- Includes undo/redo support per editor plugin best practices
- Does NOT modify the AI pathfinding runtime code (that belongs to ccgs-ai-programmer)

### Case 2: Out-of-domain request — redirects correctly
**Input:** "Implement the enemy melee combo system in code."
**Expected behavior:**
- Does NOT produce gameplay mechanic code
- Explicitly states that combat system implementation belongs to `ccgs-gameplay-programmer`
...
- Coordinates with `ccgs-engine-programmer` to establish a read-only data access pattern (e.g., a resource validation API)
...
- Does NOT change the art pipeline requirements themselves (those belong to ccgs-art-director / ccgs-technical-artist)
...
- [ ] Coordinates with ccgs-engine-programmer before accessing runtime data from editor context
...
- Runtime data access (Case 3) confirms the agent respects the ccgs-engine-programmer's ownership of core APIs
