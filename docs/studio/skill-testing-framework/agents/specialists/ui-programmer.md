# Agent Test Spec: ccgs-ui-programmer

## Agent Summary
Domain: Menu screens, HUDs, inventory screens, dialogue boxes, UI framework code, and data binding.
Does NOT own: UX flow design (ccgs-ux-designer), visual style direction (ccgs-art-director / ccgs-technical-artist).
...
- Explicitly states that UX flow design belongs to `ccgs-ux-designer`
...
**Expected behavior:**
- Recognizes that defining the animation curve and feel is within ccgs-technical-artist territory
- Does NOT invent animation parameters (timing, easing) without a spec
- Coordinates with `ccgs-technical-artist` for an animation spec (duration, easing curve, overshoot amount)
...
- Flags the ambiguity back to `ccgs-ux-designer` with the specific question: "What should the detail panel show when an empty inventory slot is selected?"
- May propose two common options (hide panel / show placeholder) to help ccgs-ux-designer decide quickly
...
- [ ] Redirects UX flow design to ccgs-ux-designer
- [ ] Coordinates with ccgs-technical-artist for animation specs before implementing animations
- [ ] Flags ambiguous UX specs back to ccgs-ux-designer rather than making arbitrary implementation decisions
