# Agent Test Spec: ccgs-engine-programmer

## Agent Summary
Domain: Rendering pipeline, physics integration, memory management, resource loading, and core engine framework.
Does NOT own: gameplay mechanics (ccgs-gameplay-programmer), editor/debug tool UI (ccgs-tools-programmer).
...
- Explicitly states that menu screens belong to `ccgs-ui-programmer`
- Redirects the request to `ccgs-ui-programmer`
- May note it can provide engine-level audio volume API endpoints for the ccgs-ui-programmer to call
...
**Expected behavior:**
- Does NOT unilaterally change the physics query API surface (would break ccgs-gameplay-programmer's code)
- Coordinates with `ccgs-lead-programmer` to plan the change safely
...
- [ ] Redirects UI/menu requests to ccgs-ui-programmer
- [ ] Returns structured findings (implementation code, diagnosis steps, migration plans)
- [ ] Coordinates with ccgs-lead-programmer before changing shared API surfaces
