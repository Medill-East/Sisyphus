# Agent Test Spec: ccgs-performance-analyst
...
- Explicitly states that implementing optimizations belongs to the appropriate programmer (ccgs-engine-programmer for rendering batching)
- Redirects the implementation to `ccgs-engine-programmer` with the recommendation context attached
- May produce a requirements brief for the optimization so ccgs-engine-programmer has a clear target
...
- Escalates the trade-off to `ccgs-lead-programmer` for a decision
...
- [ ] Escalates code-quality trade-offs to ccgs-lead-programmer rather than deciding unilaterally
