# Agent Test Spec: ccgs-gameplay-programmer

## Agent Summary
Domain: Game mechanics code, player systems, combat implementation, and interactive features.
Does NOT own: UI implementation (ccgs-ui-programmer), AI behavior trees (ccgs-ai-programmer), engine/rendering systems (ccgs-engine-programmer).
...
- Does NOT implement UI feedback (delegates to ccgs-ui-programmer) or AI reaction (delegates to ccgs-ai-programmer)
...
- Redirects the request to `ccgs-ui-programmer`
...
- Flags the threading concern to `ccgs-engine-programmer` with a clear description of the hot path
- May produce a non-threaded refactor to reduce work per frame as a safe interim step
- Documents the escalation so ccgs-lead-programmer is aware
...
- Flags the conflict to `ccgs-lead-programmer` with the ADR reference and the trade-off described
- Will implement only after explicit override decision from ccgs-lead-programmer or ccgs-technical-director
...
- [ ] Redirects out-of-domain requests to correct agent (ccgs-ui-programmer, ccgs-ai-programmer, ccgs-engine-programmer)
