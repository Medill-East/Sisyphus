# Agent Test Spec: ccgs-ai-programmer

## Agent Summary
Domain: NPC behavior, state machines, pathfinding, perception systems, and AI decision-making.
Does NOT own: player mechanics (ccgs-gameplay-programmer), rendering or engine internals (ccgs-engine-programmer).
...
- Explicitly states this is outside its domain (player mechanics belong to ccgs-gameplay-programmer)
- Redirects the request to `ccgs-gameplay-programmer`
...
- Coordinates with `ccgs-level-designer` to clarify navmesh requirements and corridor dimensions
...
**Expected behavior:**
- Recognizes that a low-level, engine-integrated data structure is within ccgs-engine-programmer's domain
- Escalates to `ccgs-engine-programmer` with a clear description of the bottleneck and required interface
- May provide the algorithmic spec (binary heap interface, expected operations) to guide the ccgs-engine-programmer
...
- [ ] Redirects out-of-domain requests to correct agent (ccgs-gameplay-programmer, ccgs-engine-programmer, ccgs-level-designer)
...
- [ ] Escalates performance-critical low-level structures to ccgs-engine-programmer
...
- Performance escalation (Case 4) confirms the agent recognizes the ccgs-engine-programmer boundary
