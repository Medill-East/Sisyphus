# Agent Test Spec: ccgs-lead-programmer
...
**Does NOT own:** Game design decisions (ccgs-game-designer), creative direction (ccgs-creative-director), production scheduling (ccgs-producer), visual art direction (ccgs-art-director).
...
Verified by reading the agent's `.codex/agents/ccgs-lead-programmer.md` frontmatter:
...
**Scenario:** Team member asks ccgs-lead-programmer to review and approve the balance formula for player damage scaling across levels, checking whether the numbers "feel right."
**Expected:** Agent declines to evaluate design balance and redirects to ccgs-systems-designer.
**Assertions:**
- [ ] Does not make any binding assessment of formula balance or game feel
- [ ] Explicitly names `ccgs-systems-designer` as the correct handler
- [ ] May note code implementation concerns about the formula (e.g., integer overflow risk at max level), but defers all balance evaluation to ccgs-systems-designer
...
**Scenario:** ccgs-game-designer wants a mechanic where every NPC maintains a full simulation of needs, schedule, and memory (similar to a full life-sim AI). ccgs-lead-programmer calculates this will exceed the frame budget by 3x at target NPC counts. ccgs-game-designer insists the mechanic is core to the game vision.
**Expected:** ccgs-lead-programmer states the specific frame budget violation with numbers, proposes alternative approaches (e.g., LOD-based simulation, simplified need model), but explicitly defers the "is this worth the cost or should the design change" decision to ccgs-creative-director as the creative arbiter.
**Assertions:**
...
- [ ] Explicitly defers the design priority decision to `ccgs-creative-director`
...
- [ ] Defers design priority conflicts to ccgs-creative-director
...
- Tech debt assessment and prioritization are not covered here — deferred to $ccgs-tech-debt skill integration.
- Coding standards document updates (adding a new forbidden pattern) are not covered.
- Interaction with ccgs-qa-lead on what constitutes a testable unit (LP vs QL boundary) is not covered.
