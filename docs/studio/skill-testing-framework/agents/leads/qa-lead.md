...
Verified by reading the agent's `.codex/agents/ccgs-qa-lead.md` frontmatter:
...
**Scenario:** A developer asks ccgs-qa-lead to implement the automated test harness for the new physics system.
**Expected:** Agent declines to implement the test code and redirects to the appropriate programmer (ccgs-gameplay-programmer or ccgs-lead-programmer).
**Assertions:**
- [ ] Does not write or propose code implementation
- [ ] Explicitly names `ccgs-lead-programmer` or `ccgs-gameplay-programmer` as the correct handler for implementation
...
**Scenario:** ccgs-gameplay-programmer and ccgs-qa-lead disagree on whether a test that asserts "enemy patrol path visits all waypoints within 5 seconds" is deterministic enough to be a valid automated test. ccgs-gameplay-programmer argues timing variability makes it flaky; ccgs-qa-lead believes it is acceptable.
**Expected:** ccgs-qa-lead acknowledges the technical flakiness concern and escalates to ccgs-lead-programmer for a technical ruling on what constitutes an acceptable determinism standard for automated tests.
**Assertions:**
- [ ] Escalates to `ccgs-lead-programmer` for the technical ruling on determinism standards
- [ ] Does not unilaterally override the ccgs-gameplay-programmer's flakiness concern
...
- [ ] Escalates technical standards disputes to ccgs-lead-programmer
...
- Bug severity triage (P0/P1/P2 classification) is not covered here — deferred to $ccgs-bug-triage skill integration.
- Release quality gate behavior (PASS / FAIL vocabulary variant) is not covered.
- Interaction between QL-STORY-READY and story Done criteria ($ccgs-story-done skill) is not covered.
