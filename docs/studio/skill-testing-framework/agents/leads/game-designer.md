# Agent Test Spec: ccgs-game-designer
...
**Does NOT own:** Code implementation (ccgs-lead-programmer / ccgs-gameplay-programmer), visual art (ccgs-art-director), narrative lore and story (ccgs-narrative-director — coordinates with), balance formula math (ccgs-systems-designer — collaborates with).
...
Verified by reading the agent's `.codex/agents/ccgs-game-designer.md` frontmatter:
...
**Scenario:** A team member asks ccgs-game-designer to write the in-world lore explanation for why the stamina system exists (e.g., the narrative reason characters have stamina limits in the game world).
**Expected:** Agent declines to write narrative/lore content and redirects to ccgs-writer or ccgs-narrative-director.
**Assertions:**
- [ ] Does not write narrative or lore content
- [ ] Explicitly names `ccgs-writer` or `ccgs-narrative-director` as the correct handler
...
**Scenario:** ccgs-systems-designer proposes a damage formula with 6 variables and complex scaling interactions, arguing it produces the best tuning granularity. ccgs-game-designer believes the formula is too complex for players to intuit and want a simpler 2-variable version.
**Expected:** ccgs-game-designer owns the conceptual rule and player experience intention ("the damage should feel understandable to players"), but defers the formula granularity question to ccgs-systems-designer. If the disagreement cannot be resolved between them (one wants complex, one wants simple), escalate to ccgs-creative-director for a player experience ruling.
**Assertions:**
- [ ] Clearly states the player experience intention (intuitive damage, player agency)
- [ ] Defers formula granularity decisions to `ccgs-systems-designer`
- [ ] Escalates unresolved disagreement to `ccgs-creative-director` for player experience arbiter ruling
- [ ] Does not unilaterally impose a formula structure on ccgs-systems-designer
...
- [ ] Escalates design-vs-formula conflicts to ccgs-creative-director when unresolved
...
- Core loop validation across multiple interconnected systems (not just a single mechanic) is not covered — deferred to $ccgs-review-all-gdds integration.
- Coordination protocol with ccgs-systems-designer on formula ownership boundary could benefit from additional cases.
