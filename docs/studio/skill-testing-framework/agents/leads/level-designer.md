# Agent Test Spec: ccgs-level-designer

## Agent Summary
**Domain owned:** Level layouts, encounter design, pacing and tension arc, environmental storytelling, spatial puzzles.
**Does NOT own:** Narrative dialogue (ccgs-writer / ccgs-narrative-director), visual art style (ccgs-art-director), code implementation (ccgs-lead-programmer / ccgs-ai-programmer), enemy AI behavior logic (ccgs-ai-programmer / ccgs-gameplay-programmer).
...
Verified by reading the agent's `.codex/agents/ccgs-level-designer.md` frontmatter:
...
**Scenario:** A team member asks ccgs-level-designer to write the behavior tree code for an enemy patrol AI that navigates the level layout.
**Expected:** Agent declines to write AI behavior code and redirects to ccgs-ai-programmer or ccgs-gameplay-programmer.
**Assertions:**
- [ ] Does not write or specify code for AI behavior logic
- [ ] Explicitly names `ccgs-ai-programmer` or `ccgs-gameplay-programmer` as the correct handler
...
**Scenario:** ccgs-game-designer wants higher encounter density throughout the level (more enemies in each room) to increase combat challenge. ccgs-level-designer believes this density undermines the pacing arc by eliminating rest periods and making the level feel relentless without reward.
**Expected:** ccgs-level-designer clearly articulates the pacing concern (eliminating rest periods removes the tension-release rhythm), acknowledges ccgs-game-designer's challenge goal, and escalates to ccgs-creative-director for a design arbiter ruling on whether challenge density or pacing rhythm takes precedence for this level.
**Assertions:**
- [ ] Articulates the specific pacing impact of increased encounter density
- [ ] Escalates to `ccgs-creative-director` as the design arbiter
- [ ] Does not unilaterally override ccgs-game-designer's challenge density request
...
- [ ] Escalates challenge-density vs. pacing conflicts to ccgs-creative-director
...
- Interaction between ccgs-level-designer and ccgs-narrative-director for environmental lore placement is not covered.
