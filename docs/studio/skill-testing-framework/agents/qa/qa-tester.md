# Agent Test Spec: ccgs-qa-tester
...
- **Does NOT own**: Test strategy and test plan design (ccgs-qa-lead), implementation fixes for found bugs (appropriate programmer), QA process architecture (ccgs-qa-lead)
- **Category**: qa
- **Model tier**: Sonnet
- **Gate IDs**: None; flags ambiguous acceptance criteria to ccgs-qa-lead rather than resolving independently
...
- States clearly: "Bug fixes are implemented by the appropriate programmer (ccgs-gameplay-programmer for save system logic); I document the bug and write regression test cases to verify the fix"
...
### Case 3: Ambiguous acceptance criterion — flag to ccgs-qa-lead
...
- Flags to ccgs-qa-lead: "The acceptance criterion 'tutorial should feel intuitive' is not testable as written; needs clarification — e.g., 'X% of first-time players complete the tutorial without using the hint button' or 'no tester requires external help to complete the tutorial in session'"
- Provides two or three concrete, measurable alternative criteria for ccgs-qa-lead to choose between
...
- [ ] Flags ambiguous acceptance criteria to ccgs-qa-lead rather than inventing a testable interpretation
...
- Case 3 (ambiguous criterion) is a coordination test — ccgs-qa-tester must not silently accept untestable criteria
