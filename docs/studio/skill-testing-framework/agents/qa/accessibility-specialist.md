# Agent Test Spec: ccgs-accessibility-specialist
...
Does NOT own: overall UX flow design (ccgs-ux-designer), visual art style direction (ccgs-art-director).
...
- Does NOT redesign the HUD — produces findings for ccgs-ux-designer and ccgs-ui-programmer to act on
...
- Explicitly states that overall game flow design belongs to `ccgs-ux-designer`
- Redirects the request to `ccgs-ux-designer`
...
- Does NOT unilaterally change the art palette (that belongs to ccgs-art-director)
- Flags the conflict to `ccgs-art-director` with the specific visual overlap described
...
- Coordinates with `ccgs-ui-programmer` to implement the required accessible text node exposure
...
- [ ] Redirects UX flow design to ccgs-ux-designer, art palette decisions to ccgs-art-director
- [ ] Returns structured findings with specific element names, contrast ratios, and criterion references
- [ ] Does not implement UI changes — coordinates with ccgs-ui-programmer for implementation
- [ ] References specific WCAG criteria by number when compliance target is provided
- [ ] Flags conflicts between accessibility requirements and art decisions to ccgs-art-director

---

## Coverage Notes
- HUD audit (Case 1) should produce findings trackable as accessibility stories in the sprint backlog
- Colorblind conflict (Case 3) confirms the agent respects ccgs-art-director's authority over the palette
