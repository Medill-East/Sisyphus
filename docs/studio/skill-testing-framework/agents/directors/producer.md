# Agent Test Spec: ccgs-producer

## Agent Summary
**Domain owned:** Scope management, sprint planning validation, milestone tracking, epic prioritization, production phase gate.
**Does NOT own:** Game design decisions (ccgs-creative-director / ccgs-game-designer), technical architecture (ccgs-technical-director), creative direction.
...
Verified by reading the agent's `.codex/agents/ccgs-producer.md` frontmatter:
...
**Scenario:** Team member asks ccgs-producer to evaluate whether the game's "weight-based inventory" mechanic feels fun and engaging.
**Expected:** Agent declines to evaluate game feel and redirects to ccgs-game-designer or ccgs-creative-director.
**Assertions:**
- [ ] Does not make any binding assessment of the mechanic's design quality
- [ ] Explicitly names `ccgs-game-designer` or `ccgs-creative-director` as the correct handler
...
**Scenario:** ccgs-game-designer wants to add a late-breaking mechanic (dynamic weather affecting all gameplay systems) that ccgs-technical-director warns will require 3 additional sprints. ccgs-game-designer and ccgs-technical-director are in disagreement about whether to proceed.
**Expected:** Producer does not take a side on whether the mechanic is worth adding (design decision) or feasible (technical decision). Producer quantifies the production impact (3 sprints of delay, milestone slip risk), presents the trade-off to the user, and follows coordination-rules.md conflict resolution: escalate to the shared parent (in this case, surface the conflict for user decision since ccgs-creative-director and ccgs-technical-director are both top-tier).
**Assertions:**
...
## Coverage Notes
- PR-EPIC (epic-level prioritization) is not covered — a dedicated case should be added when the $ccgs-create-epics skill produces structured epic documents.
- PR-MILESTONE (milestone health review) is not covered — deferred to integration with $ccgs-milestone-review skill.
