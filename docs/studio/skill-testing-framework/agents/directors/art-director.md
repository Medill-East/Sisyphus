# Agent Test Spec: ccgs-art-director
...
**Does NOT own:** UX interaction flows and information architecture (ccgs-ux-designer's domain), audio direction (ccgs-audio-director), code implementation.
**Model tier:** Sonnet (note: despite the "director" title, ccgs-art-director is assigned Sonnet per coordination-rules.md — it handles individual system analysis, not multi-document phase gate synthesis at the Opus level).
...
Verified by reading the agent's `.codex/agents/ccgs-art-director.md` frontmatter:
...
- [ ] Model tier is `claude-sonnet-4-6` (NOT Opus — coordination-rules.md assigns Sonnet to ccgs-art-director)
...
**Scenario:** Sound designer asks ccgs-art-director to specify how ambient audio should layer and duck when the player enters a combat zone.
**Expected:** Agent declines to define audio behavior and redirects to ccgs-audio-director.
**Assertions:**
- [ ] Does not make any binding decision about audio layering or ducking behavior
- [ ] Explicitly names `ccgs-audio-director` as the correct handler
- [ ] May note if the audio has visual mood implications (e.g., "the audio should match the visual tension of the zone"), but defers all audio specification to ccgs-audio-director
...
**Scenario:** ccgs-ux-designer proposes using high-contrast, brightly colored icons for the HUD to improve readability. ccgs-art-director believes this violates the art bible's muted visual language and would undermine the visual identity.
**Expected:** ccgs-art-director states the visual identity concern and references the art bible, acknowledges ccgs-ux-designer's readability goal as legitimate, and escalates to ccgs-creative-director to arbitrate the trade-off between visual coherence and usability.
**Assertions:**
- [ ] Escalates to `ccgs-creative-director` (shared parent for creative domain conflicts)
- [ ] Does not unilaterally override ccgs-ux-designer's readability recommendation
...
- [ ] Escalates UX-vs-visual conflicts to ccgs-creative-director
...
## Coverage Notes
- AD-PHASE-GATE (full visual phase advancement) is not covered — deferred to integration with $ccgs-gate-check skill.
