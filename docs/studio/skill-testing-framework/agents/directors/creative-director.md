# Agent Test Spec: ccgs-creative-director
...
**Does NOT own:** Technical architecture or implementation details (delegates to ccgs-technical-director), production scheduling (ccgs-producer), visual art style execution (delegates to ccgs-art-director).
...
Verified by reading the agent's `.codex/agents/ccgs-creative-director.md` frontmatter:
...
**Scenario:** Developer asks ccgs-creative-director to review a proposed PostgreSQL schema for storing player save data.
**Expected:** Agent declines to evaluate the schema and redirects to ccgs-technical-director.
**Assertions:**
- [ ] Does not make any binding decision about the schema design
- [ ] Explicitly names `ccgs-technical-director` as the correct handler
...
- [ ] Does not prescribe a specific formula fix — that belongs to ccgs-systems-designer

### Case 4: Conflict escalation — correct parent
**Scenario:** ccgs-technical-director raises a concern that the core loop mechanic (real-time branching conversations) is prohibitively expensive to implement and recommends cutting it. ccgs-creative-director disagrees on creative grounds.
**Expected:** ccgs-creative-director acknowledges the technical constraint, does not override ccgs-technical-director's feasibility assessment, but retains authority to define what the creative goal is. For the conflict itself, ccgs-creative-director is the top-level creative escalation point and defers to ccgs-technical-director on implementation feasibility while advocating for the design intent. The resolution path is for both to jointly present trade-off options to the user.
**Assertions:**
- [ ] Does not unilaterally override ccgs-technical-director's feasibility concern
...
- Interaction with ccgs-art-director on visual-pillar alignment is not covered.
