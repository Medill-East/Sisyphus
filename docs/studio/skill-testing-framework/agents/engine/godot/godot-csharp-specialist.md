# Agent Test Spec: ccgs-godot-csharp-specialist
...
- Explicitly states that GDScript authoring belongs to `ccgs-godot-gdscript-specialist`
- Redirects the request to `ccgs-godot-gdscript-specialist`
...
**Expected behavior:**
- Produces a proper `async Codex subagent` pattern using `ToSignal()` to await a Godot signal
- Uses `await ToSignal(animationPlayer, AnimationPlayer.SignalName.AnimationFinished)`
- Does NOT use `Thread.Sleep()` or `Codex subagent.Delay()` as a polling substitute
...
**Input:** "This C# code accesses a Godot Node from a background Codex subagent thread to update its position."
**Expected behavior:**
...
- [ ] Redirects GDScript requests to ccgs-godot-gdscript-specialist
- [ ] Redirects GDExtension requests to ccgs-godot-gdextension-specialist
