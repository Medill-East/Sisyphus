# Agent Test Spec: ccgs-ue-replication-specialist
...
- **Does NOT own**: Gameplay logic being replicated (ccgs-gameplay-programmer), server infrastructure and hosting (ccgs-devops-engineer), GAS-specific prediction (ccgs-ue-gas-specialist handles GAS net prediction)
- **Model tier**: Sonnet
- **Gate IDs**: None; escalates security-relevant replication concerns to ccgs-lead-programmer
...
- Notes that if GAS is in use, the built-in GAS prediction handles this — recommend coordinating with ccgs-ue-gas-specialist
...
- States clearly: "Server infrastructure and deployment architecture is owned by ccgs-devops-engineer; I handle the Unreal replication layer within a running game session"
...
- Notes this should be reviewed by ccgs-lead-programmer given the economy implications
...
- [ ] Redirects server infrastructure requests to ccgs-devops-engineer without producing infrastructure design
- [ ] Flags unvalidated server RPCs as security issues and recommends ccgs-lead-programmer review
...
- Case 1 GAS branch: if GAS is configured, agent should detect it and defer to ccgs-ue-gas-specialist for GAS-managed attributes
