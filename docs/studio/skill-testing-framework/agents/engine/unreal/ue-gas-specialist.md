# Agent Test Spec: ccgs-ue-gas-specialist

## Agent Summary
- **Domain**: Gameplay Ability System (GAS) — abilities (UGameplayAbility), gameplay effects (UGameplayEffect), attribute sets (UAttributeSet), gameplay tags, ability tasks (UAbilityCodex subagent), ability specs (FGameplayAbilitySpec), GAS prediction and latency compensation
- **Does NOT own**: UI display of ability state (ccgs-ue-umg-specialist), net replication of GAS data beyond built-in GAS prediction (ccgs-ue-replication-specialist), art or VFX for ability feedback (vfx-artist)
- **Model tier**: Sonnet
...
**Expected behavior**:
- Produces a GAS AbilitySpec structure or outline: UGameplayAbility subclass with ActivateAbility logic, an AbilityCodex subagent for movement (e.g., AbilityCodex subagent_ApplyRootMotionMoveToForce or custom root motion), and a UGameplayEffect for the cooldown
...
- For custom replication needs beyond GAS built-ins, explicitly states: "For custom net serialization of GAS data, coordinate with ccgs-ue-replication-specialist"
...
- [ ] Redirects custom replication requests to ccgs-ue-replication-specialist with clear explanation of boundary
