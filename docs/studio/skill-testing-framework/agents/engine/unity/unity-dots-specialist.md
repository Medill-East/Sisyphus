# Agent Test Spec: ccgs-unity-dots-specialist
...
Does NOT own: MonoBehaviour gameplay code (ccgs-gameplay-programmer), UI implementation (ccgs-unity-ui-specialist).
...
- If no commitment exists, flags the architecture decision to `ccgs-lead-programmer` / `ccgs-technical-director` for resolution
...
- [ ] Redirects MonoBehaviour-only gameplay to ccgs-gameplay-programmer
