# Agent Test Spec: ccgs-unity-ui-specialist

## Agent Summary
Domain: Unity UI Toolkit (UXML/USS), UGUI (Canvas), data binding, runtime UI performance, and UI input event handling.
Does NOT own: UX flow design (ccgs-ux-designer), visual art style (ccgs-art-director).
...
- Explicitly states that interaction flow design belongs to `ccgs-ux-designer`
- Redirects the request to `ccgs-ux-designer`
- Notes it will implement whatever flow the ccgs-ux-designer specifies
