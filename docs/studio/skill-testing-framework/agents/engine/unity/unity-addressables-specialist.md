# Agent Test Spec: ccgs-unity-addressables-specialist
...
Does NOT own: rendering systems (ccgs-engine-programmer), game logic that uses the loaded assets (ccgs-gameplay-programmer).
...
- Explicitly states that rendering system implementation belongs to `ccgs-engine-programmer`
- Redirects the request to `ccgs-engine-programmer`
...
- Does NOT design the server-side CDN infrastructure (defers to ccgs-devops-engineer)
...
- [ ] Redirects rendering and gameplay asset-use code to ccgs-engine-programmer and ccgs-gameplay-programmer
...
- [ ] Does not design CDN/server infrastructure — defers to ccgs-devops-engineer for server side
