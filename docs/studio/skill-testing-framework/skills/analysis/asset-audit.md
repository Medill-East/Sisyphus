# Skill Test Spec: $ccgs-asset-audit

## Skill Summary

`$ccgs-asset-audit` audits the `assets/` directory for naming convention compliance,
...
**Expected behavior:**
1. Skill reads GDD references to find expected assets (cross-references with `$ccgs-content-audit` scope)
...
### Case 5: Gate Compliance — No gate; ccgs-technical-artist may be consulted separately
...
- The interaction between `$ccgs-asset-audit` and `$ccgs-content-audit` (both check GDD
  references vs. assets) is intentional overlap; `$ccgs-asset-audit` focuses on
  compliance while `$ccgs-content-audit` focuses on completeness.
