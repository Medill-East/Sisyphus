# Skill Test Spec: $ccgs-balance-check

## Skill Summary

`$ccgs-balance-check` reads balance data files (JSON or YAML in `assets/data/`) and
...
3. Skill outputs: "Cannot validate economy stats — no formulas defined. Run $ccgs-design-system first."
...
- [ ] Output recommends running `$ccgs-design-system` to define formulas
