# Skill Test Spec: $ccgs-perf-profile

## Skill Summary

`$ccgs-perf-profile` is a structured performance profiling workflow that identifies
...
**Input:** `$ccgs-perf-profile production/qa/profiler-export-2026-03-15.json`
...
**Fixture:**
- User runs `$ccgs-perf-profile` with no arguments
- No profiler data files exist in `production/qa/`

**Input:** `$ccgs-perf-profile`
...
**Input:** `$ccgs-perf-profile production/qa/profiler-export-2026-03-20.json`
...
**Input:** `$ccgs-perf-profile production/qa/profiler-export-2026-04-05.json`
...
### Case 5: Gate Compliance — No gate; ccgs-performance-analyst separate

**Fixture:**
- Profiler data shows CONCERNS-level findings (some spikes)
- `review-mode.txt` contains `full`

**Input:** `$ccgs-perf-profile production/qa/profiler-export-2026-04-01.json`

**Expected behavior:**
1. Skill analyzes profiler data; verdict is CONCERNS
2. No director gate is invoked regardless of review mode
3. Output notes: "For in-depth analysis, consider running `$ccgs-perf-profile` with the ccgs-performance-analyst agent"
