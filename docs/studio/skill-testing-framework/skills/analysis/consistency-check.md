# Skill Test Spec: $ccgs-consistency-check

## Skill Summary

`$ccgs-consistency-check` scans all GDDs in `design/gdd/` and checks for internal
...
- [ ] Skill suggests running `$ccgs-design-system system-B` to create the missing GDD
...
3. Skill outputs an error: "No GDDs found in `design/gdd/`. Run `$ccgs-design-system` to create GDDs first."
...
- [ ] Skill recommends the correct next action (`$ccgs-design-system`)
...
- This skill checks for structural consistency between GDDs. Deep design theory
  analysis (pillar drift, dominant strategies) is handled by `$ccgs-review-all-gdds`.
