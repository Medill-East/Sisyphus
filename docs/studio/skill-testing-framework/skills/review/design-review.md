# Skill Test Spec: $ccgs-design-review

## Skill Summary

`$ccgs-design-review` reads a game design document (GDD) and evaluates it against
...
- [ ] Does NOT require "May I write" language (read-only skill — `allowed-tools` excludes apply_patch/edit)
...
**Input:** `$ccgs-design-review design/gdd/light-manipulation.md`
...
**Input:** `$ccgs-design-review design/gdd/[document].md`
...
**Input:** `$ccgs-design-review design/gdd/nonexistent.md`
...
**Input:** `$ccgs-design-review design/gdd/light-manipulation.md` (with full review mode active)
...
- [ ] Ends with recommended next step (e.g., fix issues and re-run, or proceed to `$ccgs-map-systems`)
...
- Cross-system consistency checking (Case 3 in the skill's own phase list) is
  ...
  this is covered by the `$ccgs-review-all-gdds` spec instead.
