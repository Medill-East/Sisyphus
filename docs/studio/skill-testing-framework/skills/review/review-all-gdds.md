# Skill Test Spec: $ccgs-review-all-gdds

## Skill Summary

`$ccgs-review-all-gdds` is an Opus-tier skill that performs a holistic cross-GDD review
...
- [ ] Next-step handoff to `$ccgs-architecture-review` or `$ccgs-create-architecture` is present
...
- [ ] Skill suggests running `$ccgs-design-system system-B` to resolve the gap
...
3. Skill recommends running `$ccgs-brainstorm` and `$ccgs-design-system` before re-running
...
- [ ] Skill recommends the correct next action (`$ccgs-brainstorm` or `$ccgs-design-system`)
...
- [ ] Ends with appropriate handoff: MAJOR ISSUES → fix and re-run; MINOR ISSUES → may proceed with awareness; CONSISTENT → `$ccgs-create-architecture`
