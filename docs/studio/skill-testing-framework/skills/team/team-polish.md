# Skill Test Spec: $ccgs-team-polish

## Skill Summary

Orchestrates the polish team through a six-phase pipeline: performance assessment
(ccgs-performance-analyst) → optimization (ccgs-performance-analyst, optionally with
ccgs-engine-programmer when engine-level root causes are found) → visual polish
(ccgs-technical-artist, parallel with Phase 2) → audio polish (ccgs-sound-designer, parallel
with Phase 2) → hardening (ccgs-qa-tester) → sign-off (orchestrator collects all results
and issues READY FOR RELEASE or NEEDS MORE WORK). Uses `Codex request_user_input` at each
...
- [ ] Has a next-step handoff at the end (references `$ccgs-release-checklist`, `$ccgs-sprint-plan update`, `$ccgs-gate-check`)
...
- [ ] ccgs-engine-programmer is conditionally spawned in Phase 2 only when Phase 1 identifies engine-level root causes
...
**Input:** `$ccgs-team-polish combat`

**Expected behavior:**
1. Phase 1: ccgs-performance-analyst is spawned; profiles the combat system, measures frame budget, checks memory usage; output: performance report showing all metrics within budget, no violations
2. `Codex request_user_input` presents performance report; user approves before Phases 2, 3, and 4 begin
3. Phase 2: ccgs-performance-analyst applies minor optimizations (e.g., draw call batching); no ccgs-engine-programmer needed (no engine-level root causes identified)
4. Phases 3 and 4 are launched in parallel alongside Phase 2:
   - Phase 3: ccgs-technical-artist reviews VFX for quality, optimizes particle systems, adds screen shake and visual juice
   - Phase 4: ccgs-sound-designer reviews audio events for completeness, checks mix levels, adds ambient audio layers
5. All three parallel phases complete; `Codex request_user_input` presents results; user approves before Phase 5 begins
6. Phase 5: ccgs-qa-tester runs edge case tests, soak tests, stress tests, and regression tests; all pass
7. `Codex request_user_input` presents test results; user approves before Phase 6
...
**Assertions:**
- [ ] ccgs-performance-analyst is spawned first in Phase 1 before any other agents
- [ ] `Codex request_user_input` appears after Phase 1 output and before Phases 2/3/4 launch
- [ ] Phases 3 and 4 Codex subagent calls are issued at the same time as Phase 2 (not after Phase 2 completes)
- [ ] ccgs-engine-programmer is NOT spawned when Phase 1 finds no engine-level root causes
- [ ] ccgs-qa-tester (Phase 5) is not launched until the parallel phases complete and user approves
...
- Phase 2 ccgs-performance-analyst applies optimizations reducing cost to 9ms — still over the 6ms budget
- Phase 2 cannot fully resolve the violation without a fundamental design change

**Input:** `$ccgs-team-polish particle-storm`

**Expected behavior:**
1. Phase 1: ccgs-performance-analyst identifies the 12ms frame cost vs. 6ms budget; reports "FRAME BUDGET VIOLATION: particle-storm costs 12ms, budget is 6ms"
2. `Codex request_user_input` presents the violation; user chooses to proceed with optimization attempt
3. Phase 2: ccgs-performance-analyst applies optimizations; achieves 9ms — reduced but still over budget; reports "Optimization reduced cost to 9ms (was 12ms) — 3ms over budget. No further gains achievable without design changes."
4. Phases 3 and 4 run in parallel with Phase 2 (visual and audio polish)
5. Phase 5: ccgs-qa-tester runs regression and edge case tests; all pass
...
9. Next Steps: schedule the remaining issue in `$ccgs-sprint-plan update`; re-run `$ccgs-team-polish` after fix
...
- [ ] Next Steps references `$ccgs-sprint-plan update` for scheduling the remaining fix
- [ ] Phases 3 and 4 still run (polish work is not abandoned due to a Phase 2 partial resolution)
- [ ] Phase 5 ccgs-qa-tester still runs (regression testing is independent of the performance outcome)
...
**Input:** `$ccgs-team-polish` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `$ccgs-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`)"
...
### Case 4: Engine-Level Bottleneck — ccgs-engine-programmer spawned conditionally in Phase 2
...
**Input:** `$ccgs-team-polish open-world`

**Expected behavior:**
1. Phase 1: ccgs-performance-analyst profiles the environment; identifies frame budget violation; root cause analysis points to engine-level rendering pipeline (spatial indexer traversal overhead)
2. Phase 1 output explicitly classifies the root cause as engine-level
3. `Codex request_user_input` presents the performance report including the engine-level root cause; user approves before Phase 2
4. Phase 2: ccgs-performance-analyst is spawned for game-code-level optimizations AND ccgs-engine-programmer is spawned in parallel for the engine-level rendering fix
5. Phases 3 and 4 also run in parallel with Phase 2 (visual and audio polish)
6. ccgs-engine-programmer addresses the spatial indexer traversal; provides profiler validation showing the fix reduces overhead
7. Phase 5: ccgs-qa-tester runs regression tests including tests for the engine-level fix
...
**Assertions:**
- [ ] ccgs-engine-programmer is NOT spawned in Phase 2 unless Phase 1 explicitly identifies an engine-level root cause
- [ ] ccgs-engine-programmer is spawned in Phase 2 when Phase 1 identifies an engine-level root cause
- [ ] ccgs-engine-programmer and ccgs-performance-analyst Codex subagent calls in Phase 2 are issued simultaneously (not sequentially)
- [ ] Phases 3 and 4 also run in parallel with Phase 2 (not deferred until Phase 2 completes)
- [ ] ccgs-engine-programmer's output includes profiler validation of the fix
- [ ] ccgs-qa-tester in Phase 5 runs regression tests that cover the engine-level change
...
- Phase 5: ccgs-qa-tester runs regression tests and finds that a shader optimization applied in Phase 3 broke the item highlight glow effect on hover — an existing feature that was working before the polish pass

**Input:** `$ccgs-team-polish inventory-ui` (Phase 5 scenario)

**Expected behavior:**
1. Phases 1–4 complete; polish changes include a shader optimization from ccgs-technical-artist
2. Phase 5: ccgs-qa-tester runs regression tests and detects "Item highlight glow on hover no longer renders — regression introduced by shader optimization in Phase 3"
3. ccgs-qa-tester returns test results with the regression noted
4. Orchestrator surfaces the regression immediately: "ccgs-qa-tester: REGRESSION FOUND — `item-highlight-hover` glow broken by Phase 3 shader optimization"
...
7. `Codex request_user_input` presents the regression with options:
...
- [ ] `Codex request_user_input` offers options including revert, fix in place, and schedule later
- [ ] Verdict is NEEDS MORE WORK when a regression is present and unresolved
- [ ] Verdict may become READY FOR RELEASE only if the regression is fixed within the current polish session and ccgs-qa-tester re-runs to confirm
...
- [ ] ccgs-engine-programmer is only spawned when Phase 1 explicitly identifies engine-level root causes
...
- [ ] Next Steps handoff references `$ccgs-release-checklist` (on success) and `$ccgs-sprint-plan update` + `$ccgs-gate-check` (on failure)

---

## Coverage Notes

- The ccgs-tools-programmer optional agent (for content pipeline tool verification) is not
  separately tested — it follows the same conditional spawn pattern as ccgs-engine-programmer
...
- Soak testing and stress testing (Phase 5) are validated implicitly by Case 1's
  ccgs-qa-tester output. Case 5 focuses on the regression detection aspect of Phase 5.
- The "minimum spec hardware" test path in Phase 5 is not separately tested — it follows
  the same ccgs-qa-tester delegation pattern when the hardware is available.
