# Skill Test Spec: $ccgs-team-ui

## Skill Summary

Orchestrates the UI team through the full UX pipeline for a single UI feature.
Coordinates ccgs-ux-designer, ccgs-ui-programmer, ccgs-art-director, the engine UI specialist,
and ccgs-accessibility-specialist through five structured phases: Context Gathering +
...
Uses `Codex request_user_input` at each phase transition. Delegates all file writes to
sub-agents and sub-skills (`$ccgs-ux-design`, `ccgs-ui-programmer`). Produces a summary report
with verdict COMPLETE / BLOCKED and handoffs to `$ccgs-ux-review`, `$ccgs-code-review`,
`$ccgs-team-polish`.
...
- [ ] Has a next-step handoff at the end (references `$ccgs-ux-review`, `$ccgs-code-review`, `$ccgs-team-polish`)
- [ ] Error Recovery Protocol section is present with all four recovery steps
- [ ] Uses `Codex request_user_input` at phase transitions for user approval before proceeding
- [ ] Phase 4 is explicitly marked as parallel (ccgs-ux-designer, ccgs-art-director, ccgs-accessibility-specialist)
- [ ] UX Review Gate (Phase 1c) is defined as a blocking gate — skill must not proceed to Phase 2 without APPROVED verdict
- [ ] Team Composition lists all five roles (ccgs-ux-designer, ccgs-ui-programmer, ccgs-art-director, engine UI specialist, ccgs-accessibility-specialist)
- [ ] References the interaction pattern library (`design/ux/interaction-patterns.md`) — ccgs-ui-programmer must use existing patterns
...
**Input:** `$ccgs-team-ui inventory screen`

**Expected behavior:**
1. Phase 1a — orchestrator reads game-concept.md, player-journey.md, relevant GDD UI sections, interaction-patterns.md, accessibility-requirements.md; summarizes a brief for the ccgs-ux-designer
2. Phase 1b — `$ccgs-ux-design inventory-screen` invoked (or ccgs-ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `Codex request_user_input` confirms spec before review
3. Phase 1c — `$ccgs-ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — ccgs-art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `Codex request_user_input` confirms before Phase 3
5. Phase 3 — engine UI specialist spawned first (read from technical-preferences.md); produces implementation notes for ccgs-ui-programmer; ccgs-ui-programmer spawned with UX spec + visual spec + engine notes; implementation produced; interaction-patterns.md updated if new patterns introduced
6. Phase 4 — ccgs-ux-designer, ccgs-art-director, ccgs-accessibility-specialist spawned in parallel; all three return results before Phase 5
...
**Assertions:**
- [ ] Phase 1a reads all five sources before briefing ccgs-ux-designer
...
- [ ] Engine UI specialist spawned before ccgs-ui-programmer in Phase 3
- [ ] Phase 4 agents launched simultaneously (ccgs-ux-designer, ccgs-art-director, ccgs-accessibility-specialist)
- [ ] All file writes delegated to sub-agents and sub-skills
- [ ] Verdict COMPLETE in final summary report
- [ ] Next steps include `$ccgs-ux-review`, `$ccgs-code-review`, `$ccgs-team-polish`
...
- `$ccgs-ux-review` returns verdict NEEDS REVISION with specific concerns flagged (e.g., gamepad navigation flow incomplete, contrast ratio below minimum)

**Input:** `$ccgs-team-ui inventory screen`

**Expected behavior:**
1. Phase 1a + 1b complete — UX spec produced
2. Phase 1c — `$ccgs-ux-review design/ux/inventory-screen.md` returns NEEDS REVISION
3. Skill does NOT advance to Phase 2
4. `Codex request_user_input` presented with the specific flagged concerns and options:
   - (a) Return to ccgs-ux-designer to address the issues and re-review
   - (b) Accept the risk and proceed to Phase 2 anyway (conscious decision)
5. If user chooses (a): ccgs-ux-designer revises spec, `$ccgs-ux-review` re-run; loop continues until APPROVED or user overrides
...
- [ ] `Codex request_user_input` presents the specific flagged concerns before offering options
...
**Input:** `$ccgs-team-ui inventory screen` (resuming from Phase 3 complete)

**Expected behavior:**
1. Phase 4 begins after implementation is confirmed complete
2. Three Codex subagent calls issued simultaneously: ccgs-ux-designer, ccgs-art-director, ccgs-accessibility-specialist
3. Each stream operates independently:
   - ccgs-ux-designer: verifies implementation matches wireframes, tests keyboard-only and gamepad-only navigation, checks accessibility features function
   - ccgs-art-director: verifies visual consistency with art bible at minimum and maximum supported resolutions
   - ccgs-accessibility-specialist: audits against the Enhanced accessibility tier in `design/accessibility-requirements.md`; any violation flagged as a blocker
4. Skill waits for all three results before proceeding to Phase 5
5. `Codex request_user_input` presents all three review results before Phase 5 begins
...
- [ ] `Codex request_user_input` shows all three review streams' results together before Phase 5 approval
...
**Input:** `$ccgs-team-ui settings menu`
...
3. `Codex request_user_input` presented with options:
   - (a) Run `$ccgs-ux-design patterns` first to establish the pattern library, then continue
   - (b) Proceed without the pattern library — ccgs-ux-designer will document new patterns as they are created
4. Skill does NOT invent or assume patterns from other sources
5. If user chooses (b): ccgs-ui-programmer is explicitly instructed to treat all patterns created as new and to add each to a new `design/ux/interaction-patterns.md` at completion
...
- [ ] `Codex request_user_input` offers a "create pattern library first" option (referencing `$ccgs-ux-design patterns`)
- [ ] If user proceeds without the library, ccgs-ui-programmer is told to treat all patterns as new
...
- [ ] `Codex request_user_input` used at each phase transition — user approves before pipeline advances
...
- [ ] Next steps present at end: `$ccgs-ux-review`, `$ccgs-code-review`, `$ccgs-team-polish`

---

## Coverage Notes

- The HUD-specific path (`$ccgs-ux-design hud` + `hud-design.md` template + visual budget check in Phase 5)
