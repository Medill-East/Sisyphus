# Skill Test Spec: $ccgs-team-audio

## Skill Summary

Orchestrates the audio team through a four-step pipeline: audio direction
(ccgs-audio-director) → sound design + accessibility review in parallel (ccgs-sound-designer
+ ccgs-accessibility-specialist) → technical implementation + engine validation in
parallel (ccgs-technical-artist + primary engine specialist) → code integration
(ccgs-gameplay-programmer). Reads relevant GDDs, the sound bible (if present), and
...
`Codex request_user_input` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] Has a next-step handoff at the end (references `$ccgs-dev-story`, `$ccgs-asset-audit`)
- [ ] Error Recovery Protocol section is present
- [ ] `Codex request_user_input` is used at step transitions before proceeding
- [ ] Step 2 explicitly spawns ccgs-sound-designer and ccgs-accessibility-specialist in parallel
- [ ] Step 3 explicitly spawns ccgs-technical-artist and engine specialist in parallel (when engine is configured)
...
2. Step 1: ccgs-audio-director is spawned; defines sonic identity, emotional tone, adaptive music direction, mix targets, and adaptive audio rules for combat
3. `Codex request_user_input` presents audio direction; user approves before Step 2 begins
4. Step 2: ccgs-sound-designer and ccgs-accessibility-specialist are spawned in parallel; ccgs-sound-designer produces SFX specifications, audio event list with trigger conditions, and mixing groups; ccgs-accessibility-specialist identifies critical gameplay audio events and specifies visual fallback and subtitle requirements
5. `Codex request_user_input` presents SFX spec and accessibility requirements; user approves before Step 3 begins
6. Step 3: ccgs-technical-artist and primary engine specialist are spawned in parallel; ccgs-technical-artist designs bus structure, middleware integration, memory budgets, and streaming strategy; engine specialist validates that the integration approach is idiomatic for the configured engine
7. `Codex request_user_input` presents technical plan; user approves before Step 4 begins
8. Step 4: ccgs-gameplay-programmer is spawned; wires up audio events to gameplay triggers, implements adaptive music, sets up occlusion zones, writes unit tests for audio event triggers
...
- [ ] ccgs-audio-director is spawned before ccgs-sound-designer or ccgs-accessibility-specialist
- [ ] `Codex request_user_input` appears after Step 1 output and before Step 2 launch
- [ ] ccgs-sound-designer and ccgs-accessibility-specialist Codex subagent calls are issued simultaneously in Step 2
- [ ] ccgs-technical-artist and engine specialist Codex subagent calls are issued simultaneously in Step 3
- [ ] ccgs-gameplay-programmer is not launched until Step 3 `Codex request_user_input` is approved
...
- ccgs-sound-designer's audio event list includes "EnemyNearbyAlert" — a spatial audio cue that warns the player an enemy is approaching from off-screen
- ccgs-accessibility-specialist reviews the event list and finds "EnemyNearbyAlert" has no visual fallback (no on-screen indicator, no subtitle, no controller rumble specified)

**Input:** `$ccgs-team-audio stealth` (Step 2 scenario)

**Expected behavior:**
1. Steps 1–2 proceed; ccgs-accessibility-specialist and ccgs-sound-designer are spawned in parallel
2. ccgs-accessibility-specialist returns its review with a BLOCKING concern: "`EnemyNearbyAlert` is a critical gameplay audio event (warns player of off-screen threat) with no visual fallback — hearing-impaired players cannot detect this threat. This is a BLOCKING accessibility gap."
3. Orchestrator surfaces the concern immediately in conversation before presenting `Codex request_user_input`
4. `Codex request_user_input` presents the accessibility concern as a BLOCKING issue with options:
...
5. Step 3 (ccgs-technical-artist + engine specialist) is not launched until the user resolves or explicitly accepts the gap
...
- [ ] `Codex request_user_input` surfaces the gap before Step 3 is launched
...
**Input:** `$ccgs-team-audio` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `$ccgs-team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`)"
...
**Input:** `$ccgs-team-audio main menu`
...
4. ccgs-audio-director in Step 1 is informed that no sound bible exists and must establish sonic identity from the feature GDD alone
...
- [ ] ccgs-audio-director is notified that no sound bible exists in its prompt context
...
**Input:** `$ccgs-team-audio boss encounter`

**Expected behavior:**
1. Context gathering: orchestrator reads `docs/studio/technical-preferences.md` and detects no engine is configured
2. Steps 1–2 proceed normally (ccgs-audio-director, ccgs-sound-designer, ccgs-accessibility-specialist)
3. Step 3: ccgs-technical-artist is spawned normally; engine specialist spawn is SKIPPED
...
5. Step 4: ccgs-gameplay-programmer proceeds with a note that engine-specific audio integration patterns could not be validated
...
- [ ] ccgs-technical-artist is still spawned in Step 3 (skip applies only to the engine specialist)
- [ ] ccgs-gameplay-programmer proceeds in Step 4 with the deferred validation noted
...
- [ ] `Codex request_user_input` is used after every step output before the next step launches
- [ ] Parallel spawning: Step 2 (ccgs-sound-designer + ccgs-accessibility-specialist) and Step 3 (ccgs-technical-artist + engine specialist) issue all Codex subagent calls before waiting for results
...
- [ ] Next Steps handoff references `$ccgs-dev-story` and `$ccgs-asset-audit`
...
- Step 4 (ccgs-gameplay-programmer) happy-path behavior is validated implicitly by Case 1.
  Failure modes for this step follow the standard Error Recovery Protocol.
- The ccgs-accessibility-specialist's subtitle and caption requirements (beyond visual fallbacks)
