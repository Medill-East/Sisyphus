# Skill Test Spec: $ccgs-team-qa

## Skill Summary

Orchestrates the QA team through a 7-phase structured testing cycle. Coordinates
ccgs-qa-lead (strategy, test plan, sign-off report) and ccgs-qa-tester (test case writing,
...
NOT APPROVED verdict. Parallel ccgs-qa-tester spawning is used in Phase 5 for
independent stories.
...
- [ ] Independent ccgs-qa-tester tasks in Phase 5 are spawned in parallel
...
2. Phase 2: Spawns `ccgs-qa-lead` via Codex subagent; produces strategy table classifying all 4 stories; no blockers flagged; presents to user; Codex request_user_input: user selects "Looks good — proceed to test plan"
...
4. Phase 4: Spawns `ccgs-qa-lead` via Codex subagent; reviews `tests/smoke/`; returns PASS; reports "Smoke check passed. Proceeding to test case writing."
5. Phase 5: Spawns `ccgs-qa-tester` via Codex subagent for each Visual/Feel and Integration story (2–3 stories); run in parallel; presents test cases grouped by story; Codex request_user_input per group; user approves
6. Phase 6: Walks through each approved story; user marks all as PASS; result summary: "Stories PASS: 4, FAIL: 0, BLOCKED: 0"
7. Phase 7: Spawns `ccgs-qa-lead` via Codex subagent to produce sign-off report; report shows all stories PASS; no bugs filed; Verdict: APPROVED; asks "May I write this QA sign-off report to `production/qa/qa-signoff-sprint-03-[date].md`?"; writes after approval
...
- [ ] Phase 5 ccgs-qa-tester tasks for independent stories are issued in parallel
...
- [ ] Next step: "Run `$ccgs-gate-check` to validate advancement."
...
**Input:** `$ccgs-team-qa sprint-04`

**Expected behavior:**
1. Phases 1–3 complete normally; QA plan is written
2. Phase 4: Spawns `ccgs-qa-lead` via Codex subagent; smoke check returns FAIL; two specific failures are identified
3. Skill reports: "Smoke check failed. QA cannot begin until these issues are resolved: [list of 2 failures]. Fix them and re-run `$ccgs-smoke-check`, or re-run `$ccgs-team-qa` once resolved."
...
- [ ] Skill recommends `$ccgs-smoke-check` and `$ccgs-team-qa` re-run as remediation steps
...
**Input:** `$ccgs-team-qa sprint-05`

**Expected behavior:**
1. Phases 1–5 complete normally; test cases are written for the Visual/Feel story
2. Phase 6: User marks Visual/Feel story as FAIL; Codex request_user_input collects failure description: "Animation plays at 2x speed — jitter visible on every loop"
3. Phase 6: Spawns `ccgs-qa-tester` via Codex subagent to write a formal bug report; bug report written to `production/qa/bugs/BUG-001-animation-speed-jitter.md` (or next increment if bugs exist); report includes severity field
4. Result summary: "Stories PASS: 1, FAIL: 1 — bugs filed: BUG-001"
5. Phase 7: Spawns `ccgs-qa-lead` to produce sign-off report; Bugs Found table lists BUG-001 with severity and status Open; Verdict: NOT APPROVED (S1/S2 bug open, or FAIL without documented workaround)
6. Sign-off report write is offered; writes after approval
7. Next step: "Resolve S1/S2 bugs and re-run `$ccgs-team-qa` or targeted manual QA before advancing."

**Assertions:**
- [ ] FAIL result in Phase 6 triggers Codex request_user_input to collect the failure description before the bug report is written
- [ ] `ccgs-qa-tester` is spawned via Codex subagent to write the bug report — orchestrator does not write it directly
...
- [ ] Next step explicitly mentions re-running `$ccgs-team-qa`
...
**Input:** `$ccgs-team-qa` (no argument)
...
3. Proceeds as if `$ccgs-team-qa sprint-06` was the input; reports "No sprint argument provided — inferred sprint-06 from session state. Found [N] stories."
...
2. Cannot infer sprint; uses Codex request_user_input: "Which sprint or feature should QA cover?" with options to type a sprint identifier or cancel
...
- [ ] When both state files are absent, skill uses Codex request_user_input rather than guessing (variant B)
...
3. After Story C FAIL: ccgs-qa-tester spawned to write bug report `BUG-001-crash-ability-activation.md` with S1 severity
4. Result summary presented: "Stories PASS: 1, PASS WITH NOTES: 1, FAIL: 1 — bugs filed: BUG-001 (S1), BLOCKED: 1"
5. Phase 7: ccgs-qa-lead produces sign-off report covering all 4 stories; BUG-001 listed as S1/Open; Story D listed as BLOCKED; Verdict: NOT APPROVED
...
- [ ] `Codex request_user_input` used at Phase 2 (strategy review), Phase 5 (test case approval per group), and Phase 6 (per-story manual QA result)
...
- [ ] Bug reports are always written by `ccgs-qa-tester` via Codex subagent — orchestrator does not write directly
- [ ] Phase 5 ccgs-qa-tester tasks for independent stories are issued in parallel where possible
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with Codex request_user_input options
...
- Parallel ccgs-qa-tester spawning in Phase 5 is validated implicitly by Case 1 (multiple Visual/Feel stories issued simultaneously); no dedicated parallelism case is required beyond the Static Assertions check.
