# Skill Test Spec: $ccgs-team-live-ops

## Skill Summary

Orchestrates the live-ops team through a 7-phase planning pipeline to produce a
season or event plan. Coordinates ccgs-live-ops-designer, ccgs-economy-designer,
ccgs-analytics-engineer, ccgs-community-manager, ccgs-narrative-director, and ccgs-writer. Phases 3
...
- [ ] Has a next-step handoff at the end referencing `$ccgs-design-review`, `$ccgs-sprint-plan`, and `$ccgs-team-release`
- [ ] Uses `Codex request_user_input` at phase transitions to capture user approval before proceeding
...
**Input:** `$ccgs-team-live-ops "Season 2: The Frozen Wastes"`

**Expected behavior:**
1. Phase 1: Spawns `ccgs-live-ops-designer` via Codex subagent; receives season brief with scope, content list, and retention mechanic; presents to user
2. Codex request_user_input: user approves Phase 1 output before Phase 2 begins
3. Phase 2: Spawns `ccgs-narrative-director` via Codex subagent; reads the Phase 1 season brief; produces narrative framing document (theme, story hook, lore connections); presents to user
4. Phase 3 and 4 (parallel): Spawns `ccgs-economy-designer` and `ccgs-analytics-engineer` simultaneously via two Codex subagent calls before waiting for either result; ccgs-economy-designer reads `design/live-ops/economy-rules.md`
5. Phase 5: Spawns `ccgs-narrative-director` and `ccgs-writer` in parallel to produce in-game narrative text and player-facing copy; both read Phase 2 narrative framing doc
6. Phase 6: Spawns `ccgs-community-manager` via Codex subagent; reads season brief, economy design, and narrative framing; produces communication calendar with draft copy
...
8. Codex request_user_input: user approves the full season plan
...
- [ ] Next steps reference `$ccgs-design-review`, `$ccgs-sprint-plan`, and `$ccgs-team-release`
...
- ccgs-economy-designer (Phase 3) proposes a "Mystery Chest" mechanic with randomized premium rewards and no pity timer

**Input:** `$ccgs-team-live-ops "Season 3: Shadow Tournament"`

**Expected behavior:**
1. Phases 1–4 proceed normally; ccgs-economy-designer proposes Mystery Chest mechanic
...
4. Codex request_user_input presented with resolution options before season plan approval is offered
...
- [ ] Codex request_user_input is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
- [ ] Output documents are NOT written while the violation is unresolved
- [ ] If user chooses to revise: skill re-spawns ccgs-economy-designer to produce a corrected design before returning to Phase 7 review
...
**Input:** `$ccgs-team-live-ops` (no argument)

**Expected behavior:**
1. Phase 1: No argument detected
2. Outputs: "Usage: `$ccgs-team-live-ops [season name or event description]` — Provide the name or description of the season or live event to plan."
...
- Phase 3 (ccgs-economy-designer) and Phase 4 (ccgs-analytics-engineer) inputs are independent of each other

**Input:** `$ccgs-team-live-ops "Season 1: The First Thaw"` (observed at Phase 3/4 transition)

**Expected behavior:**
1. After Phase 2 is approved by the user, the orchestrator issues both Codex subagent calls (ccgs-economy-designer and ccgs-analytics-engineer) before awaiting either result
2. Both agents receive the season brief as context; ccgs-analytics-engineer does NOT wait for ccgs-economy-designer output to begin
3. Economy-designer output and ccgs-analytics-engineer output are collected together before Phase 5 begins
...
- [ ] Analytics-engineer prompt does NOT include ccgs-economy-designer output as a required input (the inputs are independent)
- [ ] If ccgs-economy-designer blocks but ccgs-analytics-engineer succeeds, analytics output is preserved and the block is surfaced via Codex request_user_input
...
**Input:** `$ccgs-team-live-ops "Season 4: Desert Heat"`

**Expected behavior:**
1. Phases 1–4 proceed; ccgs-economy-designer and ccgs-analytics-engineer are given the ethics policy path but it is absent
...
- [ ] `Codex request_user_input` used at every phase transition — user approves before the next phase begins
- [ ] Phases 3 and 4 are always spawned in parallel, not sequentially
- [ ] File Write Protocol: orchestrator never calls apply_patch/edit directly — all writes are delegated to sub-agents
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with Codex request_user_input options (skip / retry / stop)
...
- [ ] Next steps always include `$ccgs-design-review`, `$ccgs-sprint-plan`, and `$ccgs-team-release`

---

## Coverage Notes

- Phase 5 parallel spawning (ccgs-narrative-director + ccgs-writer) follows the same pattern as Phases 3/4 but is not separately tested here — it uses the same parallel Codex subagent protocol validated in Case 4.
- The "economy-rules.md absent" edge case is not separately tested — it would surface as a BLOCKED result from ccgs-economy-designer and follow the standard error recovery path tested implicitly in Case 4.
