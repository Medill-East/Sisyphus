# Skill Test Spec: $ccgs-team-combat

## Skill Summary

Orchestrates the full combat team pipeline end-to-end for a single combat feature.
Coordinates ccgs-game-designer, ccgs-gameplay-programmer, ccgs-ai-programmer, ccgs-technical-artist,
ccgs-sound-designer, the primary engine specialist, and ccgs-qa-tester through six structured
phases: Design → Architecture (with engine specialist validation) → Implementation
(parallel) → Integration → Validation → Sign-off. Uses `Codex request_user_input` at each
phase transition. Delegates all file writes to sub-agents. Produces a summary report
with verdict COMPLETE / NEEDS WORK / BLOCKED and handoffs to `$ccgs-code-review`,
`$ccgs-balance-check`, and `$ccgs-team-polish`.
...
- [ ] Has a next-step handoff at the end (references `$ccgs-code-review`, `$ccgs-balance-check`, `$ccgs-team-polish`)
...
- [ ] Phase 3 is explicitly marked as parallel (ccgs-gameplay-programmer, ccgs-ai-programmer, ccgs-technical-artist, ccgs-sound-designer)
- [ ] Phase 2 includes spawning the primary engine specialist (read from `docs/studio/technical-preferences.md`)
- [ ] Team Composition lists all seven roles (ccgs-game-designer, ccgs-gameplay-programmer, ccgs-ai-programmer, ccgs-technical-artist, ccgs-sound-designer, engine specialist, ccgs-qa-tester)
...
**Input:** `$ccgs-team-combat parry and riposte system`

**Expected behavior:**
1. Phase 1 — ccgs-game-designer spawned; produces `design/gdd/parry-riposte.md` covering all 8 required sections (overview, player fantasy, rules, formulas, edge cases, dependencies, tuning knobs, acceptance criteria); asks user to approve design doc
2. Phase 2 — ccgs-gameplay-programmer + ccgs-ai-programmer spawned; produce architecture sketch with class structure, interfaces, and file list; then primary engine specialist is spawned to validate idioms; engine specialist output incorporated; `Codex request_user_input` presented with architecture options before Phase 3 begins
3. Phase 3 — ccgs-gameplay-programmer, ccgs-ai-programmer, ccgs-technical-artist, ccgs-sound-designer spawned in parallel; all four return outputs before Phase 4 begins
4. Phase 4 — integration wires together all Phase 3 outputs; tuning knobs verified as data-driven; `Codex request_user_input` confirms integration before Phase 5
5. Phase 5 — ccgs-qa-tester spawned; writes test cases from acceptance criteria; verifies edge cases; performance impact checked against budget
6. Phase 6 — summary report produced: design COMPLETE, all team members COMPLETE, test cases listed, verdict: COMPLETE
7. Next steps listed: `$ccgs-code-review`, `$ccgs-balance-check`, `$ccgs-team-polish`

**Assertions:**
- [ ] `Codex request_user_input` called at each phase gate (at minimum before Phase 3 and before Phase 5)
- [ ] Phase 3 agents launched simultaneously — no sequential dependency between ccgs-gameplay-programmer, ccgs-ai-programmer, ccgs-technical-artist, ccgs-sound-designer
- [ ] Engine specialist runs in Phase 2 before Phase 3 begins (output incorporated into architecture)
- [ ] All file writes delegated to sub-agents (orchestrator never calls apply_patch/edit directly)
- [ ] Verdict COMPLETE present in final report
- [ ] Next steps include `$ccgs-code-review`, `$ccgs-balance-check`, `$ccgs-team-polish`
...
- ccgs-ai-programmer agent returns BLOCKED because no AI system architecture ADR exists (ADR status is Proposed)

**Input:** `$ccgs-team-combat parry and riposte system`

**Expected behavior:**
1. Phase 1 — design doc found; ccgs-game-designer confirms it is valid; phase approved
2. Phase 2 — ccgs-gameplay-programmer completes architecture sketch; ccgs-ai-programmer returns BLOCKED: "ADR for AI behavior system is Proposed — cannot implement until ADR is Accepted"
3. Error Recovery Protocol triggered: "ccgs-ai-programmer: BLOCKED — AI behavior ADR is Proposed"
4. `Codex request_user_input` presented with options: (a) Skip ccgs-ai-programmer and note the gap; (b) Retry with narrower scope; (c) Stop here and run `$ccgs-architecture-decision` first
5. If user chooses (a): Phase 3 proceeds with ccgs-gameplay-programmer, ccgs-technical-artist, ccgs-sound-designer only; ccgs-ai-programmer gap noted in partial report
6. Final report produced: partial implementation documented, ccgs-ai-programmer section marked BLOCKED, overall verdict: BLOCKED
...
- [ ] Blocked reason references the ADR and suggests `$ccgs-architecture-decision`
...
**Input:** `$ccgs-team-combat parry and riposte system` (resuming from Phase 2 complete)

**Expected behavior:**
1. Phase 3 begins after architecture approval
2. All four Codex subagent calls — ccgs-gameplay-programmer, ccgs-ai-programmer, ccgs-technical-artist, ccgs-sound-designer — are issued before any result is awaited
...
**Fixture:**
- `docs/studio/technical-preferences.md` has Engine Specialists section populated (e.g., Primary: ccgs-godot-specialist)
- Architecture sketch produced by ccgs-gameplay-programmer is available
- Engine version pinned in `docs/engine-reference/godot/VERSION.md`

**Input:** `$ccgs-team-combat parry and riposte system`

**Expected behavior:**
1. Phase 2 — ccgs-gameplay-programmer produces architecture sketch
...
6. `Codex request_user_input` includes engine specialist's notes alongside the architecture sketch
...
- [ ] Next steps present at end of output: `$ccgs-code-review`, `$ccgs-balance-check`, `$ccgs-team-polish`

---

## Coverage Notes

- The NEEDS WORK verdict path (ccgs-qa-tester finds failures in Phase 5) is not separately tested
...
- "Retry with narrower scope" error recovery option is listed in assertions but its full
  recursive behavior (splitting via `$ccgs-create-stories`) is covered by the `$ccgs-create-stories` spec.
