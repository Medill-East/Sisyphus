# Skill Test Spec: $ccgs-create-architecture

## Skill Summary

`$ccgs-create-architecture` guides the user through section-by-section authoring of a
...
In `full` review mode, TD-ARCHITECTURE (ccgs-technical-director) and LP-FEASIBILITY
(ccgs-lead-programmer) spawn after the complete draft is finished. In `lean` or `solo`
...
- [ ] Has a next-step handoff at the end (`$ccgs-architecture-review` or `$ccgs-create-control-manifest`)
...
In `full` mode: TD-ARCHITECTURE (ccgs-technical-director) and LP-FEASIBILITY
(ccgs-lead-programmer) spawn in parallel after all sections are drafted and before
...
**Input:** `$ccgs-create-architecture`
...
- [ ] Next-step handoff to `$ccgs-architecture-review` or `$ccgs-create-control-manifest` is present
...
- [ ] Ends with next-step handoff: `$ccgs-architecture-review` or `$ccgs-create-control-manifest`
...
- The required section list for architecture documents is defined in the skill
  body and in the `$ccgs-architecture-review` skill — not re-enumerated here.
