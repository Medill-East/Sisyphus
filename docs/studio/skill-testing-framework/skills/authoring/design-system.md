# Skill Test Spec: $ccgs-design-system

## Skill Summary

`$ccgs-design-system` guides the user through section-by-section authoring of a Game
...
The CD-GDD-ALIGN gate (ccgs-creative-director) runs in both `full` AND `lean` modes.
...
In `full` mode: CD-GDD-ALIGN (ccgs-creative-director) gate runs after each section is
...
**Input:** `$ccgs-design-system [system-name]`
...
- [ ] Ends with next-step handoff: `$ccgs-review-all-gdds` or `$ccgs-map-systems next`
