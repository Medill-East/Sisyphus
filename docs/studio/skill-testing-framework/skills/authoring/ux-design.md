# Skill Test Spec: $ccgs-ux-design

## Skill Summary

`$ccgs-ux-design` is a guided, section-by-section UX spec authoring skill. It produces
...
The skill has no inline director gates — `$ccgs-ux-review` is the separate review step.
...
- [ ] Has a next-step handoff (e.g., `$ccgs-ux-review` to validate the completed spec)

---

## Director Gate Checks

None. `$ccgs-ux-design` has no inline director gates. `$ccgs-ux-review` is the separate
...
6. Skill suggests running `$ccgs-ux-review` as the next step
...
- [ ] Handoff to `$ccgs-ux-review` is at the end
- [ ] Verdict is COMPLETE
...
**Input:** `$ccgs-ux-design inventory-screen`
...
- [ ] Handoff suggests both `$ccgs-ux-review` and writing the missing GDD
...
**Input:** `$ccgs-ux-design`

**Expected behavior:**
1. Skill detects no screen name or argument provided
2. Skill outputs a usage error: "Screen name required. Usage: `$ccgs-ux-design [screen-name]`"
3. Skill provides examples: `$ccgs-ux-design hud`, `$ccgs-ux-design main-menu`, `$ccgs-ux-design inventory`
...
**Input:** `$ccgs-ux-design settings-menu`
...
- [ ] Ends with handoff to `$ccgs-ux-review`
...
- Interaction state enumeration (normal/hover/focus/disabled/error) is a core
  requirement of each spec; the `$ccgs-ux-review` skill checks for completeness.
