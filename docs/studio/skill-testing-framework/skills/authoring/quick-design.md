# Skill Test Spec: $ccgs-quick-design

## Skill Summary

`$ccgs-quick-design` produces a lightweight design spec for features too small to
...
for a quick-design, the skill redirects to `$ccgs-design-system` instead.
...
- [ ] Mentions scope check: redirects to `$ccgs-design-system` if scope exceeds sub-4h threshold
...
**Input:** `$ccgs-quick-design [feature-name]`
...
### Case 2: Failure Path — Scope check fails; redirected to $ccgs-design-system
...
**Input:** `$ccgs-quick-design` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Skill outputs a usage error: "No feature name specified. Usage: $ccgs-quick-design [feature-name]"
3. Skill provides an example: `$ccgs-quick-design pause-menu-settings`
...
- [ ] Full GDD review path (`$ccgs-design-system`) is mentioned as the alternative for larger features

---

## Protocol Compliance

- [ ] Scope check runs before drafting (redirects to `$ccgs-design-system` if scope too large)
...
- [ ] Ends with next-step handoff (e.g., proceed to implementation or `$ccgs-dev-story`)
