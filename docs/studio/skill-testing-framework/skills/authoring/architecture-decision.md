# Skill Test Spec: $ccgs-architecture-decision

## Skill Summary

`$ccgs-architecture-decision` guides the user through section-by-section authoring of
...
In `full` review mode, TD-ADR (ccgs-technical-director) and LP-FEASIBILITY
(ccgs-lead-programmer) gate agents spawn after the draft is complete. If both gates
...
In `full` mode: TD-ADR (ccgs-technical-director) and LP-FEASIBILITY (ccgs-lead-programmer)
...
**Input:** `$ccgs-architecture-decision rendering-approach`
...
**Input:** `$ccgs-architecture-decision [topic]`
...
**Input:** `$ccgs-architecture-decision [same-topic]`
