# Skill Test Spec: $ccgs-create-epics

## Skill Summary

`$ccgs-create-epics` reads all approved GDDs and translates them into EPIC.md files,
...
In `full` review mode, a PR-EPIC gate (ccgs-producer) runs after drafting epics and
...
- [ ] Has a next-step handoff at the end (`$ccgs-create-stories`)
...
In `full` mode: PR-EPIC (ccgs-producer) gate runs after epics are drafted and before
...
4. Skill suggests running `$ccgs-design-system` and completing GDD approval first
...
- [ ] Ends with next-step handoff: `$ccgs-create-stories [epic-slug]` per created epic
