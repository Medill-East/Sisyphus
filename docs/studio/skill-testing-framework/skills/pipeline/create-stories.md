# Skill Test Spec: $ccgs-create-stories

## Skill Summary

`$ccgs-create-stories` breaks a single epic into developer-ready story files. It reads
...
- [ ] Has a next-step handoff at the end (`$ccgs-story-readiness`, `$ccgs-dev-story`)
...
**Input:** `$ccgs-create-stories [epic-name]`
...
- [ ] Blocking note names the specific ADR number and recommends `$ccgs-architecture-decision`
...
**Input:** `$ccgs-create-stories` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs a usage error: "No epic specified. Usage: $ccgs-create-stories [epic-name]"
...
- [ ] Ends with next-step handoff: `$ccgs-story-readiness` → `$ccgs-dev-story`
...
- The story sizing rule (splitting large requirement groups) is not tested here
  — it is addressed in the `$ccgs-create-stories` skill's internal logic.
