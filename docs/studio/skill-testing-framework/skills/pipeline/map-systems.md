# Skill Test Spec: $ccgs-map-systems

## Skill Summary

`$ccgs-map-systems` decomposes a game concept into a systems index. It reads the
...
(ccgs-creative-director) and TD-SYSTEM-BOUNDARY (ccgs-technical-director) spawn in parallel
...
- [ ] Has a next-step handoff at the end (`$ccgs-design-system`)
...
In `full` mode: CD-SYSTEMS (ccgs-creative-director) and TD-SYSTEM-BOUNDARY
(ccgs-technical-director) spawn in parallel after the systems decomposition is drafted
...
2. File not found
3. Skill outputs: "No game concept found. Run `$ccgs-brainstorm` to create one, then return to `$ccgs-map-systems`."
4. Skill exits without creating systems-index.md

**Assertions:**
- [ ] Skill outputs a clear error naming the missing file path
- [ ] Skill recommends `$ccgs-brainstorm` as the next action
...
- [ ] Ends with next-step handoff: `$ccgs-design-system [next-system]`
...
- The `next` argument mode (handing off the highest-priority undesigned system to
  `$ccgs-design-system`) is not tested here — it is a post-index-creation convenience.
