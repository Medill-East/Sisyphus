# Skill Test Spec: $ccgs-create-control-manifest

## Skill Summary

`$ccgs-create-control-manifest` reads all Accepted ADRs from `docs/architecture/` and
...
- [ ] Has a next-step handoff at the end (`$ccgs-create-epics` or `$ccgs-create-stories`)
...
**Input:** `$ccgs-create-control-manifest`
...
2. Skill outputs: "No ADRs found. Run `$ccgs-architecture-decision` to create ADRs before generating the control manifest."
...
- [ ] Skill recommends `$ccgs-architecture-decision` as the next action
...
- [ ] Ends with next-step handoff: `$ccgs-create-epics` or `$ccgs-create-stories`
