# Skill Test Spec: $ccgs-patch-notes

## Skill Summary

`$ccgs-patch-notes` is a Haiku-tier skill that generates player-facing patch notes
...
### Case 2: No Changelog Found — Directed to run $ccgs-changelog first
...
2. Skill outputs: "No changelog found — run $ccgs-changelog first to generate one"
3. No patch notes are generated; no file is written

**Assertions:**
- [ ] Skill does not crash when changelog is absent
- [ ] Output explicitly directs user to run `$ccgs-changelog`
...
### Case 5: Gate Compliance — No gate; ccgs-community-manager is separate
