# Skill Test Spec: $ccgs-sprint-status

## Skill Summary

`$ccgs-sprint-status` is a Haiku-tier read-only skill that reads the current active
...
None. `$ccgs-sprint-status` is a read-only reporting skill; no gates are invoked.
...
3. Skill suggests running `$ccgs-milestone-review` or `$ccgs-sprint-plan` as next steps

**Assertions:**
- [ ] Verdict is ON TRACK or SPRINT COMPLETE when all stories are Complete
- [ ] Output notes that the sprint is fully done
- [ ] Next-step suggestion references `$ccgs-milestone-review` or `$ccgs-sprint-plan`
- [ ] No files are written

---

### Case 3: No Active Sprint File — Guidance to run $ccgs-sprint-plan
...
4. Skill suggests running `$ccgs-sprint-plan` to create one

**Assertions:**
- [ ] Skill does not error or crash when no sprint file exists
- [ ] Output clearly states no active sprint was found
- [ ] Output recommends `$ccgs-sprint-plan` as the next action
