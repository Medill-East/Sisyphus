# Skill Test Spec: $ccgs-changelog

## Skill Summary

`$ccgs-changelog` is a Haiku-tier skill that auto-generates a developer-facing
...
- [ ] Has a next-step handoff (e.g., run $ccgs-patch-notes for player-facing version)
...
### Case 3: Commit Messages Without Codex subagent IDs — Grouped by date with note
...
- The `$ccgs-patch-notes` skill should be run after `$ccgs-changelog` for player-facing
