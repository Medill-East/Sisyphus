# Skill Test Spec: $ccgs-sprint-plan

## Skill Summary

`$ccgs-sprint-plan` reads the current milestone file and backlog stories, then
...
sprint draft is compiled (ccgs-producer reviews the plan). In lean and solo modes
...
5. Skill invokes PR-SPRINT gate (full mode); ccgs-producer approves
...
3. Skill suggests running `$ccgs-create-stories` to populate the backlog
4. No gate is invoked; no file is written

**Assertions:**
- [ ] Verdict is BLOCKED
- [ ] Output contains "No unstarted stories" or equivalent message
- [ ] Output recommends `$ccgs-create-stories`
...
2. PR-SPRINT gate runs; ccgs-producer returns CONCERNS: sprint is overloaded
...
- The case where no milestone file exists is not explicitly tested; behavior
  follows the BLOCKED pattern with a suggestion to run `$ccgs-gate-check` for
