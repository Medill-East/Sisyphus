# Skill Test Spec: $ccgs-start

## Skill Summary

`$ccgs-start` is the first-time onboarding skill for new projects. It guides the
...
technical-preferences.md) and then routes to `$ccgs-setup-engine` with the chosen
...
- [ ] Has a next-step handoff at the end (routes to `$ccgs-setup-engine`)

---

## Director Gate Checks

None. `$ccgs-start` is a utility setup skill. No director agents exist yet at the
...
8. Skill routes to `$ccgs-setup-engine [chosen-engine]` to complete technical config
...
- [ ] Handoff to `$ccgs-setup-engine` occurs at the end with the chosen engine argument
...
### Case 3: Engine Choice — User picks Godot 4, routes to $ccgs-setup-engine godot
...
3. Skill explicitly routes to `$ccgs-setup-engine godot` as the next step
4. Handoff message clearly names the engine and the next skill invocation

**Assertions:**
- [ ] Handoff command is `$ccgs-setup-engine godot` (not generic `$ccgs-setup-engine`)
...
- [ ] Ends with a handoff to `$ccgs-setup-engine` with the engine name as argument
