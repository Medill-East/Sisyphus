# Skill Test Spec: $ccgs-onboard

## Skill Summary

`$ccgs-onboard` generates a contextual project onboarding summary tailored for a new
...
The skill optionally accepts a role argument (e.g., `$ccgs-onboard artist`) to tailor
...
None. `$ccgs-onboard` is a read-only orientation skill. No director gates apply.
...
4. Next-step suggestions are appropriate for Production stage (e.g., `$ccgs-sprint-status`,
   `$ccgs-dev-story`)
...
### Case 2: Fresh Project — No engine, no sprint, suggests $ccgs-start
...
3. Output explains the onboarding workflow: `$ccgs-start` → `$ccgs-setup-engine` → `$ccgs-brainstorm`
4. Skill suggests running `$ccgs-start` as the immediate next step
5. Verdict is ONBOARDING COMPLETE (informational, not a failure)

**Assertions:**
- [ ] Output explicitly mentions the project is not yet configured
- [ ] `$ccgs-start` is recommended as the next step
...
3. Skill provides remediation: "Run `$ccgs-start` to initialize the project configuration"
4. No partial summary is generated

**Assertions:**
- [ ] Error message clearly identifies the missing file as AGENTS.md
- [ ] Remediation step (`$ccgs-start`) is explicitly named
...
- Git history reading is assumed available; offline/no-git scenarios are not
  tested here.
- Discipline roles beyond "artist" (e.g., programmer, designer, ccgs-producer) follow
