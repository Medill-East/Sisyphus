# Skill Test Spec: $ccgs-bug-triage

## Skill Summary

`$ccgs-bug-triage` reads all open bug reports in `production/bugs/` and produces a
...
director gates apply. The output is intended to help a ccgs-producer or QA lead
...
- [ ] Has a next-step handoff (e.g., `$ccgs-bug-report` to create new reports, `$ccgs-hotfix` for critical bugs)

---

## Director Gate Checks

None. `$ccgs-bug-triage` is a read-only advisory skill. No director gates apply.
...
### Case 2: No Bug Reports Found — Guidance to run $ccgs-bug-report
...
3. Skill suggests running `$ccgs-bug-report` to create a bug report
4. No triage table is produced

**Assertions:**
- [ ] Output explicitly states no bugs were found
- [ ] `$ccgs-bug-report` is suggested as the next step
