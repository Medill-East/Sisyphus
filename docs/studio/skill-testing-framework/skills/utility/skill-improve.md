# Skill Test Spec: $ccgs-skill-improve

## Skill Summary

`$ccgs-skill-improve` runs an automated test-fix-retest improvement loop on a skill
file. It invokes `$ccgs-skill-test static` (and optionally `$ccgs-skill-test category`) to
...
- [ ] Has a next-step handoff (e.g., run `$ccgs-skill-test spec` to validate behavioral compliance)

---

## Director Gate Checks

None. `$ccgs-skill-improve` is a meta-utility skill. No director gates apply.
...
**Input:** `$ccgs-skill-improve some-skill`

**Expected behavior:**
1. Skill runs `$ccgs-skill-test static some-skill` — baseline: 5/7 checks pass
...
5. Fixes applied; `$ccgs-skill-test static some-skill` re-run — now 7/7 checks pass
...
**Input:** `$ccgs-skill-improve gate-check`
...
**Input:** `$ccgs-skill-improve brainstorm`

**Expected behavior:**
1. Skill runs `$ccgs-skill-test static brainstorm` — 7/7 checks pass
...
- The improvement loop is designed to run only one fix-retest cycle per
  invocation; running multiple iterations requires re-invoking `$ccgs-skill-improve`.
