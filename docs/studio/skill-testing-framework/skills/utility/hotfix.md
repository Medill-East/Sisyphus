# Skill Test Spec: $ccgs-hotfix

## Skill Summary

`$ccgs-hotfix` manages an emergency fix workflow: it creates a hotfix branch from
main, applies a targeted fix to the identified file(s), runs `$ccgs-smoke-check` to
...
- [ ] Has a next-step handoff (e.g., `$ccgs-bug-report` to document the issue, or version bump)
...
**Input:** `$ccgs-hotfix` (user describes the crash and affected file)
...
5. Skill runs `$ccgs-smoke-check` — PASS
...
- [ ] `$ccgs-smoke-check` runs after the fix is applied
...
- `$ccgs-smoke-check` returns FAIL: "Player health clamping regression detected"

**Input:** `$ccgs-hotfix`

**Expected behavior:**
1. Skill applies the fix and runs `$ccgs-smoke-check`
...
**Fixture:**
- User invokes `$ccgs-hotfix` with a vague description: "something is broken on level 3"
- No repro steps provided

**Input:** `$ccgs-hotfix` (vague description)
...
- [ ] Runs `$ccgs-smoke-check` after applying the fix
