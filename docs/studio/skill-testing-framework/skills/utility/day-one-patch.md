# Skill Test Spec: $ccgs-day-one-patch

## Skill Summary

`$ccgs-day-one-patch` prepares a day-one patch plan for issues that are known at
...
triggers guidance to run `$ccgs-hotfix` before the patch. No director gates apply.
...
- [ ] Has a next-step handoff (e.g., `$ccgs-hotfix` for P0 issues, `$ccgs-release-checklist` for follow-up)

---

## Director Gate Checks

None. `$ccgs-day-one-patch` is a release planning utility. No director gates apply.
...
### Case 2: Critical Issue Discovered Post-Ship — P0, Triggers $ccgs-hotfix Guidance
...
4. Skill explicitly directs: "Run `$ccgs-hotfix` to resolve this issue first"
...
- [ ] `$ccgs-hotfix` is explicitly directed for the P0 issue
...
- [ ] Escalates CRITICAL (P0) bugs with explicit `$ccgs-hotfix` guidance
...
- The patch notes player communication document (`$ccgs-patch-notes`) is a separate
