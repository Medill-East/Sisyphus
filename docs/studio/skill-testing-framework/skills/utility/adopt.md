# Skill Test Spec: $ccgs-adopt

## Skill Summary

`$ccgs-adopt` audits an existing project's artifacts — GDDs, ADRs, stories, infrastructure
...
after explicit user approval via `Codex request_user_input`.

This skill is distinct from `$ccgs-project-stage-detect` (which checks what exists).
`$ccgs-adopt` checks whether what exists will actually work with the template's skills.
...
Verified automatically by `$ccgs-skill-test static` — no fixture needed.

- [ ] Has required frontmatter fields: `name`, `description`, `argument-hint`, `user-invocable`, `allowed-tools`
- [ ] Has ≥2 phase headings
- [ ] Contains severity tier keywords: BLOCKING, HIGH, MEDIUM, LOW
- [ ] Contains "May I write" or `Codex request_user_input` language before writing the adoption plan
...
None. `$ccgs-adopt` is a brownfield audit utility. No director gates apply.
...
8. Uses `Codex request_user_input` to ask about writing the plan; user selects write
...
- [ ] `Codex request_user_input` is used before writing the adoption plan
...
4. Phase 4 builds ordered migration plan:
   - Step 1 (BLOCKING): Add `## Status` to `adr-0001.md` — command: `$ccgs-architecture-decision retrofit`
   - Step 2 (HIGH): Run `$ccgs-architecture-review` to bootstrap tr-registry.yaml
   - Step 3 (HIGH): Add Acceptance Criteria to `combat.md` — command: `$ccgs-design-system retrofit`
...
6. `Codex request_user_input` asks to write the plan; writes after approval
...
- [ ] `Codex request_user_input` is used before writing
...
6. `Codex request_user_input` to write plan; writes after approval
...
### Case 4: No Artifacts Found — Fresh project, guidance to run $ccgs-start
...
3. Uses `Codex request_user_input`:
   ...
   - Options: "Run `$ccgs-start`", "My artifacts are in a non-standard location", "Cancel"
4. Skill stops — does not proceed to audit regardless of user selection

**Assertions:**
- [ ] `Codex request_user_input` is used (not a plain text message) when no artifacts are found
- [ ] `$ccgs-start` is presented as a named option
...
4. No `$ccgs-gate-check` is invoked during the skill run
...
- [ ] Uses `Codex request_user_input` before writing the adoption plan file
