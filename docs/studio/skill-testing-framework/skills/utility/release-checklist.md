# Skill Test Spec: $ccgs-release-checklist

## Skill Summary

`$ccgs-release-checklist` generates an internal release readiness checklist covering:
...
(that is `$ccgs-launch-checklist`). When a previous release checklist exists, it shows
...
after a "May I write" ask. No director gates apply — `$ccgs-gate-check` handles
...
- [ ] Has a next-step handoff (e.g., `$ccgs-launch-checklist` for external or `$ccgs-gate-check` for phase)

---

## Director Gate Checks

None. `$ccgs-release-checklist` is an internal audit utility. Formal phase advancement
is managed by `$ccgs-gate-check`.
