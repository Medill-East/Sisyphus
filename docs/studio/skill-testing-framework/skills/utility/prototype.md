# Skill Test Spec: $ccgs-prototype

## Skill Summary

`$ccgs-prototype` manages a rapid prototyping workflow for validating a game mechanic
...
- [ ] Has a next-step handoff (e.g., `$ccgs-design-system` to formalize, or archive)
...
**Input:** `$ccgs-prototype grapple-hook`
...
**Input:** `$ccgs-prototype grapple-hook` (prototype session complete)
...
2. Recommendation in findings.md: "Mechanic validated — recommend proceeding
   to `$ccgs-design-system` for full specification"
3. Skill handoff message explicitly suggests `$ccgs-design-system grapple-hook`
4. Verdict is PROTOTYPE COMPLETE

**Assertions:**
- [ ] `findings.md` contains an explicit recommendation
- [ ] Recommendation references `$ccgs-design-system` when mechanic is validated
...
**Input:** `$ccgs-prototype procedural-dialogue`
...
**Input:** `$ccgs-prototype wall-jump`
