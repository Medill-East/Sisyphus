# Skill Test Spec: $ccgs-setup-engine

## Skill Summary

`$ccgs-setup-engine` configures the project's engine, language, rendering backend,
...
(e.g., `$ccgs-setup-engine godot`) to skip the engine-selection step. For each
...
- [ ] Has a next-step handoff (e.g., `$ccgs-brainstorm` or `$ccgs-start` depending on flow)

---

## Director Gate Checks

None. `$ccgs-setup-engine` is a technical configuration skill. No director gates apply.
...
4. Skill drafts all engine sections: engine/language/rendering/physics fields,
   ...
   (ccgs-godot-specialist, gdscript-specialist, ccgs-godot-shader-specialist, etc.)
5. Skill populates the routing table: `.gd` → gdscript-specialist, `.gdshader` →
   ccgs-godot-shader-specialist, `.tscn` → ccgs-godot-specialist
...
3. Specialist assignments reference ccgs-unity-specialist, csharp-specialist
4. Routing table: `.cs` → csharp-specialist, `.asmdef` → ccgs-unity-specialist,
   `.unity` (scene) → ccgs-unity-specialist
...
**Input:** `$ccgs-setup-engine unreal`

**Expected behavior:**
1. Skill sets engine to Unreal Engine 5, primary language to Blueprint (Visual Scripting)
2. Specialist assignments reference ccgs-unreal-specialist, blueprint-specialist
3. Routing table: `.uasset` → blueprint-specialist or ccgs-unreal-specialist,
   `.umap` → ccgs-unreal-specialist
...
- Godot 4 + C# (instead of GDScript) follows the same flow as Case 1 with
  different naming conventions and the ccgs-godot-csharp-specialist assignment.
