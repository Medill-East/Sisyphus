# Skill Test Spec: $ccgs-test-helpers

## Skill Summary

`$ccgs-test-helpers` generates engine-specific test helper utilities for the project's
...
None. `$ccgs-test-helpers` is a scaffolding utility. No director gates apply.
...
**Input:** `$ccgs-test-helpers player-factory`
...
### Case 2: No Test Setup Exists — Redirects to $ccgs-test-setup
...
3. Skill suggests running `$ccgs-test-setup` before generating helpers
4. No helper file is created

**Assertions:**
- [ ] Error message identifies the missing tests/ directory
- [ ] `$ccgs-test-setup` is suggested as the prerequisite step
...
**Input:** `$ccgs-test-helpers enemy-factory`
...
**Input:** `$ccgs-test-helpers inventory-factory`
