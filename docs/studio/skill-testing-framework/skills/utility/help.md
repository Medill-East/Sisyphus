# Skill Test Spec: $ccgs-help

## Skill Summary

`$ccgs-help` analyzes what has been done and what comes next in the project workflow.
...
guidance summary. The skill optionally accepts a context query (e.g., `$ccgs-help testing`)
...
None. `$ccgs-help` is a read-only navigation skill. No director gates apply.
...
3. Skill outputs: current stage, sprint summary, and 3 suggested next skills
   (e.g., `$ccgs-sprint-status`, `$ccgs-dev-story`, `$ccgs-story-done`)
...
3. Suggested skills are: `$ccgs-brainstorm`, `$ccgs-map-systems` (if concept exists)
...
- [ ] Suggestions do not include Production-stage skills (e.g., `$ccgs-dev-story`)
- [ ] Verdict is HELP COMPLETE
...
4. Primary suggestion is `$ccgs-start` to begin configuration

**Assertions:**
- [ ] Skill does not crash when stage.txt is absent
- [ ] Full workflow overview is shown when stage cannot be determined
- [ ] `$ccgs-start` or `$ccgs-project-stage-detect` is a top suggestion
- [ ] Verdict is HELP COMPLETE
...
**Input:** `$ccgs-help testing`

**Expected behavior:**
1. Skill reads context query: "testing"
2. Skill surfaces skills relevant to testing: `$ccgs-qa-plan`, `$ccgs-smoke-check`,
   `$ccgs-regression-suite`, `$ccgs-test-setup`, `$ccgs-test-evidence-review`
...
- [ ] General sprint skills (e.g., `$ccgs-sprint-plan`) are not the primary suggestions
- [ ] Verdict is HELP COMPLETE
...
- The case where the active sprint is complete (all stories Done) is not
  separately tested; the skill would suggest `$ccgs-sprint-plan` for the next sprint.
- The `$ccgs-help` skill does not validate whether suggested skills are available —
  it assumes standard skill catalog availability.
- Stage detection fallback (when stage.txt is absent) delegates to the same
  logic as `$ccgs-project-stage-detect` and is not re-tested here in detail.
