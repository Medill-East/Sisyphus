# Skill Test Spec: $ccgs-localize

## Skill Summary

`$ccgs-localize` manages the full localization pipeline: it extracts all player-facing
...
None. `$ccgs-localize` is a pipeline utility. No director gates apply. Localization
...
5. Skill suggests fixing the file manually and re-running `$ccgs-localize`
