# Skill Test Spec: $ccgs-smoke-check

## Skill Summary

`$ccgs-smoke-check` is the gate between implementation and QA hand-off. It detects the
test environment, runs the automated test suite (via Bash), scans test coverage
against sprint stories, and uses `Codex request_user_input` to batch-verify manual smoke
...
- [ ] Has a next-step handoff (e.g., `$ccgs-bug-report` on FAIL, QA hand-off guidance on PASS)

---

## Director Gate Checks

None. `$ccgs-smoke-check` is a pre-QA utility skill. No director gates apply.
...
5. Uses `Codex request_user_input` for Batch 1 (core stability) and Batch 2 (sprint mechanics)
...
- [ ] `Codex request_user_input` is used for manual smoke check batches
...
7. Delivers FAIL verdict with message: "The smoke check failed. Do not hand off to
   ...
   then re-running `$ccgs-smoke-check`

**Assertions:**
- [ ] Failing test names are listed in the report
- [ ] Verdict is FAIL
- [ ] Post-verdict message directs developer to fix failures before QA hand-off
- [ ] `$ccgs-smoke-check` re-run is suggested after fixing

---

### Case 3: Manual Confirmation — Codex request_user_input used, PASS WITH WARNINGS
...
3. `Codex request_user_input` is used for Batch 1 and Batch 2 — developer confirms all PASS
...
5. Verdict is PASS WITH WARNINGS — build ready for QA, but MISSING entry must be
   resolved before `$ccgs-story-done` closes the affected story
6. Asks to write report; writes after approval

**Assertions:**
- [ ] `Codex request_user_input` is used for manual smoke check batches (not inline text prompts)
- [ ] MISSING test coverage entry appears in the report
- [ ] Verdict is PASS WITH WARNINGS (not PASS, not FAIL)
- [ ] Advisory note explains MISSING entry must be resolved before `$ccgs-story-done`
...
2. Skill outputs: "No test directory found at `tests/`. Run `$ccgs-test-setup` to
...
- [ ] `$ccgs-test-setup` is suggested as the remediation step
...
4. No `$ccgs-gate-check` is invoked
...
- [ ] Uses `Codex request_user_input` for all manual smoke check batches (Batch 1, Batch 2, Batch 3)
...
- The `--platform` argument adds platform-specific Codex request_user_input batches and a
