# Skill Test Spec: $ccgs-project-stage-detect

## Skill Summary

`$ccgs-project-stage-detect` automatically analyzes project artifacts to determine
...
when `$ccgs-gate-check` passes and the user confirms advancement. The skill reports
...
- [ ] Has a next-step handoff (e.g., `$ccgs-gate-check` to formally advance stage)

---

## Director Gate Checks

None. `$ccgs-project-stage-detect` is a read-only detection utility. No director
gates apply.
...
**Input:** `$ccgs-project-stage-detect`
...
5. Next step: continue with `$ccgs-sprint-plan` or `$ccgs-dev-story`
...
5. Skill recommends running `$ccgs-gate-check` to formalize and write stage.txt

**Assertions:**
- [ ] Inferred stage is Production
- [ ] Confidence is MEDIUM (not HIGH, since stage.txt is absent)
- [ ] Recommendation to run `$ccgs-gate-check` is present
...
5. Skill suggests `$ccgs-start` to begin the onboarding workflow

**Assertions:**
- [ ] Inferred stage is Concept
- [ ] Output lists the artifacts that were checked (and found absent)
- [ ] `$ccgs-start` is suggested as the next step
- [ ] No files are written
...
5. Skill suggests reviewing stage.txt manually or running `$ccgs-gate-check`
...
- [ ] Does not write stage.txt (that is `$ccgs-gate-check`'s responsibility)
