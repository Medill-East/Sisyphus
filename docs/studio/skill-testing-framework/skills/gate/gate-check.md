# Skill Test Spec: $ccgs-gate-check

## Skill Summary

`$ccgs-gate-check` validates whether the project is ready to advance to the next
...
**Input:** `$ccgs-gate-check systems-design`
...
5. Skill suggests remediation: run `$ccgs-brainstorm` to create one
...
- [ ] Output recommends `$ccgs-brainstorm` as the remediation action
...
**Input:** `$ccgs-gate-check` (no argument)
...
**Input:** `$ccgs-gate-check systems-design` (with full mode active)

**Expected behavior:**
1. Skill reads review mode — determines `full`
2. Skill spawns all 4 PHASE-GATE director prompts in parallel:
   - CD-PHASE-GATE (ccgs-creative-director)
   - TD-PHASE-GATE (ccgs-technical-director)
   - PR-PHASE-GATE (ccgs-producer)
   - AD-PHASE-GATE (ccgs-art-director)
...
**Input:** `$ccgs-gate-check systems-design` (with solo mode active)
...
Codex request_user_input to confirm the auto-detected transition before running full checks —
