...
**Playtest note:** Run `$ccgs-playtest-report` to structure additional session data
before running `$ccgs-gate-check pre-production`.

**Next steps:**
1. `$ccgs-gate-check pre-production` — formally advance to Production
2. `$ccgs-create-epics layer:foundation` — plan Foundation layer epics
3. `$ccgs-create-epics layer:core` — plan Core layer epics
4. `$ccgs-sprint-plan` — use velocity data from this report in the estimate
...
**Architecture decisions to revisit:** [List — use `$ccgs-architecture-decision` to update]
**Core loop change needed:** [What specifically to change]

**Next steps:**
1. `$ccgs-design-system [mechanic]` — revise affected GDDs
2. `$ccgs-architecture-decision [decision]` — address architecture issues
3. `$ccgs-vertical-slice` — re-validate after revisions
...
**Next step:** `$ccgs-brainstorm` to explore a new direction, or `$ccgs-prototype [new-concept]`
