...
2. **Ask architecture questions:**
   ...
   - *Use `Codex request_user_input` to batch constrained architecture questions*
...
5. **Get approval before writing files:**
   ...
   - Wait for "yes" before using apply_patch/edit tools

6. **Complete the story with `$ccgs-story-done`:**
   - When implementation (and tests, if written) is complete, invoke `$ccgs-story-done [story-file-path]`
   - This verifies acceptance criteria, checks for GDD/ADR deviations, prompts code review, and updates the story status
   - Do NOT mark a story complete manually — always go through `$ccgs-story-done`
   - If no story file exists for this work (ad-hoc task), offer `$ccgs-code-review` directly instead
...
File is ready. Would you like me to:
 ...
 C) Run $ccgs-code-review for architecture validation"
...
You: [creates tests/combat/test_damage_calculator.gd]
     "Created test file with 6 test cases covering the acceptance criteria from the design doc.
      ...
      Running $ccgs-story-done to verify acceptance criteria and close out the story."

[$ccgs-story-done runs — verifies criteria, checks deviations, prompts code review, updates story status]
```
...
- Story completion is explicit — use `$ccgs-story-done` to close every story, never assume done because code is written

#### Structured Decision UI

Use the `Codex request_user_input` tool for architecture decisions and next-step choices.
...
- When running as a Codex subagent subagent — structure text for orchestrator

**Example — architecture questions (batch):**

  Codex request_user_input with questions:
