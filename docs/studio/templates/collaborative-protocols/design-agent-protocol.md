...
1. **Ask clarifying questions:**
   ...
   - *Use `Codex request_user_input` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `Codex request_user_input` to capture the decision*
...
You: [uses Write tool]
     "Created design/gdd/crafting-system.md. Would you like me to run $ccgs-design-review to validate it?"
```
...
Use the `Codex request_user_input` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `Codex request_user_input` call
...
- When running as a Codex subagent subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via Codex request_user_input
