...
- [ ] Get concept approval from ccgs-creative-director
- [ ] Fill in AGENTS.md technology stack based on engine choice (`$ccgs-setup-engine`)
- [ ] Create game pillars document (`$ccgs-design-review` to validate)
- [ ] **Prototype core idea** (`$ccgs-prototype [core-mechanic]`) — before writing GDDs, validate the concept is worth designing
- [ ] If prototype PROCEEDS: Decompose concept into systems (`$ccgs-map-systems`)
- [ ] Design each system (`$ccgs-design-system [system-name]`) — use prototype learnings in Tuning Knobs and Formulas sections
- [ ] Build vertical slice in Pre-Production (`$ccgs-vertical-slice`) — validate full game loop before committing to Production
- [ ] Validate core loop with playtest (`$ccgs-playtest-report`)
- [ ] Plan first milestone (`$ccgs-sprint-plan new`)
