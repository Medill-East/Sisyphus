...
**Validation:** Run `$ccgs-design-review [path]` after authoring any GDD.
Run `$ccgs-review-all-gdds` after completing a set of related GDDs.

## Quick Specs (`design/quick-specs/`)

Lightweight specs for tuning changes, minor mechanics, or balance adjustments.
Use `$ccgs-quick-design` to author.
...
Use `$ccgs-ux-design` to author. Validate with `$ccgs-ux-review` before passing to `$ccgs-team-ui`.
